(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const p of o.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&a(p)}).observe(document,{childList:!0,subtree:!0});function s(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(n){if(n.ep)return;n.ep=!0;const o=s(n);fetch(n.href,o)}})();const V={};function N(e,t){V[e]=t}function tt(e){window.location.hash=e}function st(e){if(V[e])return{handler:V[e],params:{}};for(const t in V){const s=t.split("/").filter(Boolean),a=e.split("/").filter(Boolean);if(s.length!==a.length)continue;const n={};let o=!0;for(let p=0;p<s.length;p++)if(s[p].startsWith(":"))n[s[p].slice(1)]=a[p];else if(s[p]!==a[p]){o=!1;break}if(o)return{handler:V[t],params:n}}return null}function Ie(){const e=window.location.hash.slice(1)||"/",t=st(e);if(t){const s=document.getElementById("page-content");s&&(s.innerHTML="",t.handler(s,t.params)),document.querySelectorAll(".sidebar-link").forEach(a=>{const n=a.getAttribute("data-route");n===e||e.startsWith(n)&&n!=="/"?a.classList.add("active"):a.classList.remove("active")})}else{const s=document.getElementById("page-content");s&&(s.innerHTML=`
      ...
      <div class="dashboard-widgets">
          ...
          <div class="widget-body">
            ...
            ${i.map(d=>{const I=c.getById(l.CLIENTS,d.clientId),S=G(d.firstSessionDate);return`
                <div class="widget-item clickable-row" onclick="window.location.hash='/cases/${d.id}'">
                  ...
                  <div>
                    <div class="widget-item-date ${S<=1?"text-accent":""}">${U(d.firstSessionDate)}</div>
            ...
            ${g.map(d=>{const I=c.getById(l.CASES,d.caseId),S=G(d.nextSessionDate);return`
                <div class="widget-item clickable-row" onclick="window.location.hash='/cases/${d.caseId}'">
                  ...
                  <div>
                    <div class="widget-item-date ${S<=1?"text-accent":""}">${U(d.nextSessionDate)}</div>
                    <div class="text-xs ${S<=1?"text-accent":"text-secondary"}">${S===0?"اليوم":S===1?"غداً":`خلال ${S} أيام`}</div>
                  </div>
                </div>
              `}).join("")}
        ...
        <div class="widget">
          ...
          <div class="widget-body">
            ...
            `:""}
            ${u.filter(d=>d.status==="مفتوح").sort((d,I)=>new Date(d.endDate)-new Date(I.endDate)).map(d=>{const I=c.getById(l.CASES,d.caseId),S=G(d.endDate),D=S<0?"badge-blocked":S<=3?"badge-progress":"badge-open";return`
                  <div class="widget-item clickable-row" onclick="window.location.hash='/cases/${d.caseId}'">
                    <div class="widget-item-info">
                    ...
                    <div>
                      <div class="widget-item-date">${U(d.endDate)}</div>
                      <span class="badge ${D}">${S<0?"متأخر":S===0?"اليوم":`${S} يوم`}</span>
                    </div>
                  </div>
                `}).join("")}
          </div>
  ...
  `;function s(){const a=document.getElementById("case-search").value.toLowerCase(),n=document.getElementById("filter-type").value,o=document.getElementById("filter-status").value;let p=t;a&&(p=p.filter(h=>{const r=c.getById(l.CLIENTS,h.clientId);return h.caseNo.includes(a)||h.subject.toLowerCase().includes(a)||h.opponentName.toLowerCase().includes(a)||h.court.toLowerCase().includes(a)||r&&r.name.toLowerCase().includes(a)})),n&&(p=p.filter(h=>h.caseType===n)),o&&(p=p.filter(h=>h.status===o));const u=document.getElementById("case-table-body");if(p.length===0){u.innerHTML='<tr><td colspan="9"><div class="empty-state"><p>لا توجد قضايا</p></div></td></tr>';return}u.innerHTML=p.map(h=>{const r=c.getById(l.CLIENTS,h.clientId),m=c.getById(l.USERS,h.ownerId),f=pe[h.caseType]||"civil",$=_e[h.status]||"active";return`
        ...
        <form id="case-form">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select class="form-select" id="case-type" required>
                ...
                ${je.map(r=>`<option value="${r}" ${a?.caseType===r?"selected":""}>${r}</option>`).join("")}
              </select>
            </div>
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="case-status">
              ${Ue.map(r=>`<option value="${r}" ${a?.status===r?"selected":""}>${r}</option>`).join("")}
            </select>
          </div>
          `:""}
              ...
              <div class="timeline-item">
                ...
                <div class="timeline-content">
                  <div class="flex justify-between items-center">
                    <div class="timeline-date">${U(i.date)}</div>
                  ...
                  ${i.nextSessionDate?`<div class="text-xs text-secondary mt-2">الجلسة التالية: ${U(i.nextSessionDate)}</div>`:""}
      ...
      <div class="tab-panel" id="tab-actions">
        ...
        ${r.map(i=>{const b=c.getById(l.USERS,i.responsibleUserId),E=i.clientId?c.getById(l.CLIENTS,i.clientId):null,d=Pe[i.status]||"open",I=i.dueDate&&ne(i.dueDate)&&i.status!=="مكتمل",S=ae(),D=R(),A=i.status!=="مكتمل"&&(ae()||D&&i.responsibleUserId===D.id),k=St(i.id),q=k.length===0?'<div class="text-xs text-secondary" style="padding:var(--space-2) 0">لا توجد سجلات تعديل</div>':k.map(v=>{const x=v.changes,T=new Date(v.timestamp).toLocaleString("ar-EG",{dateStyle:"short",timeStyle:"short"});if(v.action==="field_change"&&x&&x.field)return`<div class="action-history-entry">
            ...
            <div class="card mb-4 ${I?"risk-flag high":""}" style="border-right: 3px solid ${i.status==="مكتمل"?"var(--status-completed)":i.status==="معلق"?"var(--status-blocked)":"var(--status-progress)"};"
              ...
              ${i.dueDate?`<div class="text-xs text-secondary mt-1">تاريخ الاستحقاق: ${U(i.dueDate)}</div>`:""}
              ${i.executionDate?`<div class="text-xs text-accent mt-1">تم التنفيذ: ${U(i.executionDate)}</div>`:""}
        ...
        ${m.map(i=>{const b=c.getById(l.USERS,i.responsibleUserId),E=Re[i.status]||"open",d=G(i.endDate),I=i.status==="مفتوح"&&d<0,S=i.status==="مفتوح"&&d>=0&&d<=3;return`
            <div class="card mb-4" style="border-right: 3px solid ${I?"var(--risk-high)":S?"var(--risk-medium)":i.status==="مكتمل"?"var(--status-completed)":"var(--status-open)"};">
              ...
              <div class="flex gap-6 text-sm text-secondary">
                <span>من: ${U(i.startDate)}</span>
                <span>إلى: ${U(i.endDate)}</span>
  ...
  `,y=`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
  `;_(`${m?"إغلاق":u?"تعديل":"إضافة"} ${r}`,f,{footer:y,large:!0});function g(){const i=document.getElementById("session-decision").value,b=ue(i),E=document.getElementById("session-action-preview"),d=document.getElementById("action-preview-text"),I=document.getElementById("next-date-required"),S=document.getElementById("closure-reason-group"),D=document.getElementById("next-date-group"),A=b?b.requiresNextDate:!1;b?(E.style.display="block",d.textContent=`سيتم إنشاء إجراء تلقائي: ${b.actionType}`,b.subTasks?.length>0&&(d.textContent+=` (${b.subTasks.length} مهام فرعية)`)):E.style.display="none",A?(D.style.display="block",I.style.display="inline",S.style.display="none"):i?(D.style.display="block",I.style.display="none",m?S.style.display="block":S.style.display="none"):(D.style.display="block",I.style.display="inline",S.style.display="none")}document.getElementById("session-decision").addEventListener("change",g),g(),document.getElementById("save-session-btn").addEventListener("click",()=>{const i=document.getElementById("session-date").value,b=document.getElementById("session-type").value,E=document.getElementById("session-decision").value,d=document.getElementById("session-next-date").value,I=document.getElementById("session-closure-reason")?.value||"",S=document.getElementById("session-notes").value,D=[];i||D.push("تاريخ الجلسة مطلوب"),b||D.push("نوع الجلسة مطلوب"),m&&!E&&D.push("لا يمكن إغلاق الجلسة بدون تسجيل القرار/النتيجة"),!m&&!E&&D.push("نتيجة القرار مطلوبة");const A=ue(E),k=A?A.requiresNextDate:!1;if(k&&!d&&D.push("تاريخ الجلسة التالية مطلوب لهذا النوع من القرار"),m&&!k&&E&&!d&&!I&&D.push("يجب اختيار سبب عدم وجود جلسة تالية لإغلاق الجلسة"),D.length>0){const B=document.getElementById("session-form-errors");B.style.display="block",B.innerHTML=D.join("<br>");return}const q=m?"مغلق":o?.status||"مفتوح",v=Ee({caseId:e,date:i,sessionType:b,decisionResult:E,nextSessionDate:d,status:q,closureReason:m?I:o?.closureReason||"",notes:S});let x;if(u?(c.update(l.SESSIONS,o.id,v),C(l.SESSIONS,o.id,"update",v),x={...o,...v}):(x=c.create(l.SESSIONS,v),C(l.SESSIONS,x.id,"create",v)),d&&c.query(l.SESSIONS,L=>L.caseId===e&&L.date===d&&L.id!==x.id).length===0){const L=Ee({caseId:e,date:d,sessionType:b,decisionResult:"",nextSessionDate:"",status:"مفتوح",closureReason:"",notes:"جلسة تالية – تم إنشاؤها تلقائياً"}),M=c.create(l.SESSIONS,L);C(l.SESSIONS,M.id,"create",{auto:!0,fromSession:x.id})}let T=!1;if(A)try{if(c.query(l.ACTIONS,L=>L.sessionId===x.id&&L.actionType===A.actionType).length===0){const L=c.getById(l.CASES,e),M=ve({caseId:e,sessionId:x.id,actionType:A.actionType,responsibleUserId:L?.ownerId||"",status:"مفتوح",subTasks:A.subTasks?A.subTasks.map(F=>({...F})):[],dueDate:d||"",notes:A.executionProof?`إثبات التنفيذ المطلوب: ${A.executionProof}`:""}),H=c.create(l.ACTIONS,M);C(l.ACTIONS,H.id,"create",{auto:!0,decision:E,sessionId:x.id}),T=!0,console.log("Action automatically created:",H)}else console.log("Action of this type already exists for this session, skipping creation.")}catch(B){console.error("Error creating action:",B),w("حدث خطأ أثناء إنشاء الإجراء التلقائي","error")}w(T?`تم ${m?"إغلاق":"حفظ"} الجلسة وإنشاء إجراء: ${A.actionType}`:m?"تم إغلاق الجلسة بنجاح":"تم حفظ الجلسة","success"),!u&&Bt(E)&&c.getById(l.CASES,e)&&w("يمكنك الآن إنشاء قضية محكمة مرتبطة من صفحة القضايا","info",5e3),O(),J(a,n)})}function Pt(e,t,s){const a=c.getById(l.ACTIONS,e);if(!a)return;const n=`
  ...
  `;function m(){const i=document.getElementById("action-search").value.toLowerCase(),b=document.getElementById("filter-action-type").value,E=document.getElementById("filter-action-status").value,d=document.getElementById("filter-action-scope").value,I=a?"":document.getElementById("filter-responsible")?.value||"";let S=n;b&&(S=S.filter(k=>k.actionType===b)),E&&(S=S.filter(k=>k.status===E)),d==="case"&&(S=S.filter(k=>!!k.caseId)),d==="client"&&(S=S.filter(k=>!k.caseId)),I&&(S=S.filter(k=>k.responsibleUserId===I)),i&&(S=S.filter(k=>{const q=k.caseId?c.getById(l.CASES,k.caseId):null,v=k.clientId?c.getById(l.CLIENTS,k.clientId):null;return k.actionType.toLowerCase().includes(i)||k.title&&k.title.toLowerCase().includes(i)||k.notes&&k.notes.toLowerCase().includes(i)||q&&(q.caseNo.includes(i)||q.subject.toLowerCase().includes(i))||v&&v.name.toLowerCase().includes(i)}));const D={};S.forEach(k=>{D[k.actionType]||(D[k.actionType]=[]),D[k.actionType].push(k)});const A=document.getElementById("actions-container");if(Object.keys(D).length===0){A.innerHTML='<div class="empty-state"><p>لا توجد إجراءات</p></div>';return}A.innerHTML=Object.entries(D).map(([k,q])=>`
      <div class="action-group">
        <div class="action-group-header">
          <span class="action-group-icon"><i class='bx bxs-zap'></i></span>
          <span class="action-group-title">${k}</span>
          <span class="action-group-count">${q.length}</span>
        </div>
        <div class="table-container">
          <table class="data-table">
            <thead>
  ...
  `;function s(){const a=document.getElementById("filter-dl-type").value,n=document.getElementById("filter-dl-status").value;let o=t;a&&(o=o.filter(u=>u.deadlineType===a)),n&&(o=o.filter(u=>u.status===n)),o.sort((u,h)=>new Date(u.endDate)-new Date(h.endDate));const p=document.getElementById("dl-table-body");if(o.length===0){p.innerHTML='<tr><td colspan="8"><div class="empty-state"><p>لا توجد مواعيد نهائية</p></div></td></tr>';return}p.innerHTML=o.map(u=>{const h=c.getById(l.CASES,u.caseId),r=c.getById(l.USERS,u.responsibleUserId),m=Re[u.status]||"open",f=G(u.endDate),$=u.status==="مفتوح"&&f<0,y=u.status==="مفتوح"&&f>=0&&f<=3;return`
      ...
      `}).join("")}s(),document.getElementById("filter-dl-type").addEventListener("change",s),document.getElementById("filter-dl-status").addEventListener("change",s)}function fe(e){if(j("ربط القرارات بالإجراءات"),!Q()){e.innerHTML='<div class="empty-state"><h3>غير مصرح</h3><p>هذه الصفحة متاحة للشركاء فقط</p></div>';return}const t=ye();e.innerHTML=`
    <div class="animate-fade-in">
      <div class="page-header">
        <div>
  ...
  `,e.querySelector("#add-user-btn").addEventListener("click",()=>Le(null,e)),e.querySelectorAll(".edit-user").forEach(s=>{s.addEventListener("click",()=>{const a=c.getById(l.USERS,s.dataset.id);a&&Le(a,e)})}),e.querySelectorAll(".send-invite").forEach(s=>{s.addEventListener("click",()=>Yt(s.dataset.id,s.dataset.email))})}async function Yt(e,t){const s=Ye();try{const a=await fetch(`${zt}/auth/send-invite`,{method:"POST",headers:{"Content-Type":"application/json",...s?{Authorization:`Bearer ${s}`}:{}},body:JSON.stringify({userId:e})}),n=await a.json();if(!a.ok){w(n.error||"فشل إرسال الدعوة","error");return}const o=`
        ...
        </div>`;_("رابط الدعوة",o,{footer:`<button class="btn btn-primary" onclick="document.getElementById('active-modal')?.remove()">إغلاق</button>`})}catch{w("تعذر الاتصال بالخادم","error")}}function Le(e,t){const s=!!e,a=`
    <form>
      <div class="form-group">
  ...
  `}function Jt(e){if(j("إعدادات النظام"),!Q()){e.innerHTML='<div class="empty-state"><h3>غير مصرح</h3><p>هذه الصفحة متاحة للشركاء فقط</p></div>';return}se(e)}function se(e,t="notifications"){const s=c.getSetting("workdayEndTime")||"17:00",a=Ct(),n=Nt();e.innerHTML=`
    <div class="animate-fade-in">
      <div class="page-header">
        <div>
    ...
    </div>`,e.querySelectorAll(".settings-tab-btn").forEach(o=>{o.addEventListener("click",()=>se(e,o.dataset.tab))}),e.querySelector("#save-settings-btn")?.addEventListener("click",()=>{const o=document.getElementById("workday-end-time").value;if(!o){w("وقت نهاية يوم العمل مطلوب","error");return}c.setSetting("workdayEndTime",o),w("تم حفظ الإعدادات بنجاح","success")}),Be(e,"action",l.LOOKUP_ACTION_TYPES,()=>se(e,"actions")),Be(e,"decision",l.LOOKUP_DECISION_TYPES,()=>se(e,"decisions"))}function Ae(e,t,s,a){return`
    <div class="card">
        ...
        <div class="table-actions">
          ...
          <button class="btn btn-ghost btn-sm ${e}-delete-btn" data-id="${t.id}"
            ...
            `).join("");F>0&&(z+=`<button class="cal-more-btn" data-date="${B}">+${F} المزيد</button>`),A+=`
                <div class="cal-day ${M?"cal-day-today":""}" data-date="${B}"
                     style="user-select:none;">
                    <div class="cal-day-number">${v}</div>
                    <div class="cal-day-events">${z}</div>
                </div>
            `}const k=[];for(let v=1;v<=E;v++){const x=String(o+1).padStart(2,"0"),T=String(v).padStart(2,"0"),B=`${n}-${x}-${T}`,L=m[B];L&&L.length>0&&k.push({dateStr:B,day:v,events:L})}const q=k.length>0?k.map(({dateStr:v,day:x,events:T})=>`<div class="cal-agenda-day">
    ...
    `;const a=e.querySelector("#btn-scan"),n=e.querySelector("#drive-folder-id"),o=e.querySelector("#loading-overlay"),p=e.querySelector("#scan-results-card"),u=e.querySelector("#import-table-body"),h=e.querySelector("#btn-import-selected"),r=e.querySelector("#selectAllDetected");let m=[];a.addEventListener("click",async()=>{const y=n.value.trim();if(!y){w("الرجاء إدخال رقم تعريف المجلد (Folder ID)","error");return}c.setSetting("drive_pdf_folder_id",y),o.style.display="flex";try{const i=await fetch(`http://localhost:3000/api/scan-drive-pdfs?folderId=${encodeURIComponent(y)}`),b=await i.json();if(!i.ok)throw new Error(b.error||"حدث خطأ أثناء المسح");m=b.clients||[],m.length===0?(w("لم يتم العثور على أي بيانات عملاء في المجلد.","warning"),p.style.display="none"):(w(`تم اكتشاف ${m.length} عميل`,"success"),f(m),p.style.display="block")}catch(g){console.error(g),w(g.message,"error")}finally{o.style.display="none"}});function f(y){u.innerHTML="",y.forEach((i,b)=>{const E=document.createElement("tr");E.innerHTML=`
            ...
            `,u.appendChild(E)}),$(),e.querySelectorAll(".client-checkbox").forEach(i=>{i.addEventListener("change",$)})}r.addEventListener("change",y=>{e.querySelectorAll(".client-checkbox").forEach(i=>{i.checked=y.target.checked}),$()});function $(){const y=Array.from(e.querySelectorAll(".client-checkbox")).some(g=>g.checked);h.disabled=!y}h.addEventListener("click",()=>{const y=e.querySelectorAll(".client-checkbox");let g=0;y.forEach(i=>{if(i.checked){const b=i.getAttribute("data-index"),E=e.querySelector(`#name-${b}`).value.trim(),d=e.querySelector(`#nid-${b}`).value.trim(),I=m[b],S=ze({name:E||I.name,nationalId:d||I.nationalId,notes:`تم إضافته من المسح الآلي للملف: ${I.sourceFile}`,driveFolderUrl:I.driveFolderUrl||"",driveFolderId:I.sourceFileId||""}),D=c.create(l.CLIENTS,S);C(l.CLIENTS,D.id,"create",S),g++}}),w(`تم استيراد ${g} عميل بنجاح`,"success"),setTimeout(()=>{window.location.hash="/clients"},1500)})}const Qt="/api";function es(){const e=document.getElementById("auth-root");e.classList.remove("hidden"),e.innerHTML=`
    <div class="auth-bg">
