(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const p of o.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&a(p)}).observe(document,{childList:!0,subtree:!0});function s(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(i){if(i.ep)return;i.ep=!0;const o=s(i);fetch(i.href,o)}})();const X={};function q(e,t){X[e]=t}function nt(e){window.location.hash=e}function it(e){if(X[e])return{handler:X[e],params:{}};for(const t in X){const s=t.split("/").filter(Boolean),a=e.split("/").filter(Boolean);if(s.length!==a.length)continue;const i={};let o=!0;for(let p=0;p<s.length;p++)if(s[p].startsWith(":"))i[s[p].slice(1)]=a[p];else if(s[p]!==a[p]){o=!1;break}if(o)return{handler:X[t],params:i}}return null}function Ee(){const e=window.location.hash.slice(1)||"/",t=it(e);if(t){const s=document.getElementById("page-content");s&&(s.innerHTML="",t.handler(s,t.params)),document.querySelectorAll(".sidebar-link").forEach(a=>{const i=a.getAttribute("data-route");i===e||e.startsWith(i)&&i!=="/"?a.classList.add("active"):a.classList.remove("active")})}else{const s=document.getElementById("page-content");s&&(s.innerHTML=`
  ...
  `,e.querySelectorAll(".sidebar-link").forEach(i=>{i.addEventListener("click",()=>{window.innerWidth<=768&&xe()})})}function Te(e){j("لوحة التحكم");const t=U(),s=t?ee(t):null,a=s==="lawyer"||s==="trainee",i=c.getAll(l.CASES),o=c.getAll(l.SESSIONS);let p=c.getAll(l.ACTIONS);a&&t&&(p=p.filter(d=>d.responsibleUserId===t.id));const r=c.getAll(l.DEADLINES),f=c.getAll(l.CLIENTS),u=i.filter(d=>d.status==="نشطة").length,b=p.filter(d=>d.status!=="مكتمل").length,g=r.filter(d=>d.status==="مفتوح").length,x=new Date;x.setHours(0,0,0,0);const y=new Date(x);y.setDate(y.getDate()+7);const h=o.filter(d=>{if(!d.nextSessionDate)return!1;const I=new Date(d.nextSessionDate);return I>=x&&I<=y}).sort((d,I)=>new Date(d.nextSessionDate)-new Date(I.nextSessionDate)),n=i.filter(d=>{if(!d.firstSessionDate)return!1;const I=new Date(d.firstSessionDate);return I>=x&&I<=y}),v={};p.filter(d=>d.status!=="مكتمل").forEach(d=>{v[d.actionType]||(v[d.actionType]=[]),v[d.actionType].push(d)});const $=[];h.forEach(d=>{const I=J(d.nextSessionDate);if(I<=3){const S=p.filter(L=>L.sessionId===d.id&&L.status!=="مكتمل");if(S.length>0){const L=c.getById(l.CASES,d.caseId);$.push({level:"high",icon:"<i class='bx bxs-circle'></i>",text:`جلسة خلال ${I} أيام مع ${S.length} إجراء مفتوح – القضية ${L?L.caseNo:""}/${L?L.year:""}`})}}}),p.filter(d=>d.actionType==="حضور تجديد حبس"&&d.status!=="مكتمل").forEach(d=>{const I=c.getById(l.CASES,d.caseId);$.push({level:"high",icon:"<i class='bx bxs-bell-ring'></i>",text:`إجراء حبس احتياطي مفتوح – القضية ${I?I.caseNo:""}/${I?I.year:""}`})}),p.filter(d=>d.status!=="مكتمل"&&d.dueDate&&ie(d.dueDate)).forEach(d=>{const I=c.getById(l.CASES,d.caseId);$.push({level:"medium",icon:"<i class='bx bx-error'></i>",text:`إجراء متأخر: ${d.actionType} – القضية ${I?I.caseNo:""}/${I?I.year:""}`})}),r.filter(d=>d.status==="مفتوح"&&d.endDate&&ie(d.endDate)).forEach(d=>{const I=c.getById(l.CASES,d.caseId);$.push({level:"high",icon:"<i class='bx bxs-circle'></i>",text:`موعد نهائي متأخر: ${d.deadlineType} – القضية ${I?I.caseNo:""}/${I?I.year:""}`})}),e.innerHTML=`
      ...
      <div class="dashboard-widgets">
        ...
        <div class="widget">
          ...
          <div class="widget-body">
            ...
            `:""}
            ${n.map(d=>{const I=c.getById(l.CLIENTS,d.clientId),S=J(d.firstSessionDate);return`
                <div class="widget-item clickable-row" onclick="window.location.hash='/cases/${d.id}'">
                  ...
                  <div>
                    <div class="widget-item-date ${S<=1?"text-accent":""}">${P(d.firstSessionDate)}</div>
                    <div class="text-xs ${S<=1?"text-accent":"text-secondary"}">${S===0?"اليوم":S===1?"غداً":`خلال ${S} أيام`}</div>
                  </div>
                </div>
              `}).join("")}
            ${h.map(d=>{const I=c.getById(l.CASES,d.caseId),S=J(d.nextSessionDate);return`
                <div class="widget-item clickable-row" onclick="window.location.hash='/cases/${d.caseId}'">
                  ...
                  <div>
                    <div class="widget-item-date ${S<=1?"text-accent":""}">${P(d.nextSessionDate)}</div>
                    <div class="text-xs ${S<=1?"text-accent":"text-secondary"}">${S===0?"اليوم":S===1?"غداً":`خلال ${S} أيام`}</div>
                  </div>
                </div>
              `}).join("")}
        ...
        <div class="widget">
          ...
          <div class="widget-body">
            ${r.filter(d=>d.status==="مفتوح").length===0?`
              <div class="empty-state" style="padding: var(--space-6);">
                <p>لا توجد مواعيد نهائية مفتوحة</p>
              </div>
            `:""}
            ${r.filter(d=>d.status==="مفتوح").sort((d,I)=>new Date(d.endDate)-new Date(I.endDate)).map(d=>{const I=c.getById(l.CASES,d.caseId),S=J(d.endDate),L=S<0?"badge-blocked":S<=3?"badge-progress":"badge-open";return`
                  <div class="widget-item clickable-row" onclick="window.location.hash='/cases/${d.caseId}'">
                    <div class="widget-item-info">
                    ...
                    <div>
                      <div class="widget-item-date">${P(d.endDate)}</div>
        ...
        <div class="widget">
          ...
          <div class="widget-body">
            ${i.slice(-5).reverse().map(d=>{const I=c.getById(l.CLIENTS,d.clientId),S=me[d.caseType]||"civil";return`
                <div class="widget-item clickable-row" onclick="window.location.hash='/cases/${d.id}'">
                  <div class="widget-item-info">
  ...
  `,a.appendChild(o),o.querySelector("#modal-close-btn").addEventListener("click",O),o.addEventListener("click",r=>{r.target===o&&O()});const p=r=>{r.key==="Escape"&&(O(),document.removeEventListener("keydown",p))};return document.addEventListener("keydown",p),o}function O(){const e=document.getElementById("active-modal");e&&(e.style.opacity="0",setTimeout(()=>e.remove(),150))}function We(e,t,s){const a=`
  ...
  `});o.querySelector("#confirm-yes").addEventListener("click",()=>{s(),O()}),o.querySelector("#confirm-no").addEventListener("click",O)}function N(e,t,s,a={}){const i=U(),o={entityType:e,entityId:t,action:s,userId:i?i.id:"system",userName:i?i.name:"النظام",timestamp:new Date().toISOString(),changes:a};return c.create(l.AUDIT,o),o}function St(e,t,s,a=""){const i=U(),o=new Date().toISOString(),p={actionType:"نوع الإجراء",title:"العنوان / الوصف",dueDate:"تاريخ الاستحقاق",responsibleUserId:"المحامي المسؤول",priority:"الأولوية",notes:"الملاحظات",clientId:"العميل",caseId:"القضية",executionDate:"تاريخ التنفيذ",executionDetails:"تفاصيل التنفيذ / الإثبات",status:"الحالة"},r=["actionType","responsibleUserId","clientId","caseId","executionDate","executionDetails"],f=[];return Object.keys(p).forEach(u=>{const b=String(t[u]||""),g=String(s[u]||"");if(b===g)return;const x=r.includes(u),y={entityType:l.ACTIONS,entityId:e,action:"field_change",userId:i?i.id:"system",userName:i?i.name:"النظام",timestamp:o,changes:{field:u,fieldLabel:p[u]||u,oldValue:b,newValue:g,sensitive:x,editReason:x?a:""}};c.create(l.AUDIT,y),f.push(y)}),f}function wt(e){return c.query(l.AUDIT,t=>t.entityId===e).sort((t,s)=>new Date(s.timestamp)-new Date(t.timestamp))}function Tt(e=50){return c.getAll(l.AUDIT).sort((t,s)=>new Date(s.timestamp)-new Date(t.timestamp)).slice(0,e)}function kt(e){return{create:"إنشاء",update:"تعديل",complete:"إكمال",delete:"حذف",status_change:"تغيير حالة",field_change:"تعديل حقل"}[e]||e}function Xe(e){j("العملاء");const t=c.getAll(l.CLIENTS);e.innerHTML=`
    <div class="animate-fade-in">
  ...
  `,e.querySelector("#client-search").addEventListener("input",s=>{const a=s.target.value.toLowerCase(),i=t.filter(o=>o.name.toLowerCase().includes(a)||o.nationalId.includes(a)||o.phone.includes(a));document.getElementById("client-table-body").innerHTML=ke(i),Le()}),e.querySelector("#add-client-btn").addEventListener("click",()=>{window.location.hash="/clients/new"}),Le()}function ke(e){return e.length===0?'<tr><td colspan="7"><div class="empty-state"><p>لا يوجد عملاء</p></div></td></tr>':e.map(t=>{const s=t.driveFolderUrl?`<a href="${t.driveFolderUrl}" target="_blank" class="btn btn-ghost btn-sm" title="فتح مجلد درايف"><i class='bx bxl-google-drive text-blue-500'></i></a>`:"";return`
  ...
  `}).join("")}function Le(){document.querySelectorAll(".edit-client").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),window.location.hash=`/clients/${e.dataset.id}/edit`})}),document.querySelectorAll(".delete-client").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),We("حذف العميل","هل أنت متأكد من حذف هذا العميل؟",()=>{c.softDelete(l.CLIENTS,e.dataset.id),N(l.CLIENTS,e.dataset.id,"delete"),k("تم حذف العميل","success"),window.location.hash="/clients",Xe(document.getElementById("page-content"))})})})}function De(e,t={}){const s=t.id&&t.id!=="new",a=s?c.getById(l.CLIENTS,t.id):null;j(s?"تعديل بيانات العميل":"إضافة عميل جديد"),e.innerHTML=`
    <div class="animate-fade-in">
      <div class="page-header">
        <div>
  ...
  `,e.querySelector("#client-form").addEventListener("submit",p=>{p.preventDefault();const r=Ke({name:document.getElementById("client-name").value.trim(),nationalId:document.getElementById("client-national-id").value.trim(),phone:document.getElementById("client-phone").value.trim(),address:document.getElementById("client-address").value.trim(),poaNumber:document.getElementById("client-poa").value.trim(),notaryOffice:document.getElementById("client-notary").value.trim(),poaDate:document.getElementById("client-poa-date").value,notes:document.getElementById("client-notes").value.trim(),driveFolderUrl:a?.driveFolderUrl||"",driveFolderId:a?.driveFolderId||""});if(!r.name||!r.nationalId||!r.phone||!r.poaNumber||!r.notaryOffice||!r.poaDate){k("يرجى ملء جميع الحقول المطلوبة","error");return}if(s)c.update(l.CLIENTS,t.id,r),N(l.CLIENTS,t.id,"update",r),k("تم تحديث بيانات العميل","success");else{const f=c.create(l.CLIENTS,r);N(l.CLIENTS,f.id,"create",r),k("تم إنشاء العميل بنجاح","success")}window.location.hash="/clients"});const i=e.querySelector("#btn-sync-drive");async function o(p=!1){if(!a?.driveFolderId)return;const r=i?i.innerHTML:"";i&&(i.innerHTML="<i class='bx bx-loader-alt bx-spin'></i> جاري المزامنة...",i.disabled=!0);try{const u=await fetch(`/api/sync-drive?folderId=${encodeURIComponent(a.driveFolderId)}`),b=await u.json();if(!u.ok)throw new Error(b.error||"حدث خطأ أثناء المزامنة");b.nationalId?(document.getElementById("client-national-id").value=b.nationalId,k("تم العثور على الرقم القومي تلقائياً من ملفات درايف!","success"),i&&(i.style.display="none")):p||k("لم يتم العثور على رقم قومي في صور المجلد.","warning")}catch(f){console.error(f),p||k(f.message,"error")}finally{i&&(i.innerHTML=r,i.disabled=!1)}}i&&(i.addEventListener("click",()=>{o(!1)}),s&&!a?.nationalId&&o(!0))}function Lt(e){j("القضايا");const t=c.getAll(l.CASES);c.getAll(l.CLIENTS),c.getAll(l.USERS),e.innerHTML=`
    <div class="animate-fade-in">
      <div class="page-header">
        <div>
        ...
        ${$t("createCase")?`<button class="btn btn-primary" onclick="window.location.hash='/cases/new'"><i class='bx bx-plus'></i> إضافة قضية</button>`:""}
      </div>
      ...
      <div class="filter-bar">
        ...
        <select class="filter-select" id="filter-status">
          ...
          ${Re.map(a=>`<option value="${a}">${a}</option>`).join("")}
        </select>
      </div>
  ...
  `;function s(){const a=document.getElementById("case-search").value.toLowerCase(),i=document.getElementById("filter-type").value,o=document.getElementById("filter-status").value;let p=t;a&&(p=p.filter(f=>{const u=c.getById(l.CLIENTS,f.clientId);return f.caseNo.includes(a)||f.subject.toLowerCase().includes(a)||f.opponentName.toLowerCase().includes(a)||f.court.toLowerCase().includes(a)||u&&u.name.toLowerCase().includes(a)})),i&&(p=p.filter(f=>f.caseType===i)),o&&(p=p.filter(f=>f.status===o));const r=document.getElementById("case-table-body");if(p.length===0){r.innerHTML='<tr><td colspan="9"><div class="empty-state"><p>لا توجد قضايا</p></div></td></tr>';return}r.innerHTML=p.map(f=>{const u=c.getById(l.CLIENTS,f.clientId),b=c.getById(l.USERS,f.ownerId),g=me[f.caseType]||"civil",x=He[f.status]||"active";return`
        ...
        <div class="card" style="max-width: 900px;">
          ...
          <div class="form-row-3">
            ...
            <div class="form-group">
              ...
              <select class="form-select" id="case-stage" required>
                ...
                ${ct.map(u=>`<option value="${u}" ${a?.stageType===u?"selected":""}>${u}</option>`).join("")}
              </select>
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select class="form-select" id="case-type" required>
                ...
                ${_e.map(u=>`<option value="${u}" ${a?.caseType===u?"selected":""}>${u}</option>`).join("")}
              </select>
            </div>
            <div class="form-group" id="criminal-stage-group" style="display: ${a?.caseType==="جنائي"?"block":"none"};">
              ...
              <select class="form-select" id="case-criminal-stage">
                ...
                ${dt.map(u=>`<option value="${u}" ${a?.criminalStageType===u?"selected":""}>${u}</option>`).join("")}
              </select>
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-group" style="grid-column: 1 / -1;">
              ...
              <div class="client-tags" id="client-tags-container">
                ${(a?.clientIds||(a?.clientId?[a.clientId]:[])).map(u=>{const b=i.find(x=>x.id===u),g=a?.primaryClientId===u;return b?`<span class="client-tag ${g?"primary":""}" data-client-id="${u}">${b.name}${g?" (رئيسي)":""}<button class="client-tag-remove" data-remove-id="${u}">&times;</button></span>`:""}).join("")}
              </div>
              <div class="flex gap-2">
          ...
          <div class="form-group" id="primary-client-group" style="display: ${(a?.clientIds?.length||0)>1?"block":"none"};">
            ...
            <div id="primary-client-radios">
              ${(a?.clientIds||[]).map(u=>{const b=i.find(g=>g.id===u);return b?`<label class="primary-select-radio"><input type="radio" name="primary-client" value="${u}" ${a?.primaryClientId===u?"checked":""} />${b.name}</label>`:""}).join("")}
            </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select class="form-select" id="case-client-role" required>
                ...
                ${Se.map(u=>`<option value="${u}" ${a?.clientRole===u?"selected":""}>${u}</option>`).join("")}
              </select>
            </div>
          </div>
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <select class="form-select" id="case-opponent-role" required>
                <option value="">اختر الصفة</option>
                ${Se.map(u=>`<option value="${u}" ${a?.opponentRole===u?"selected":""}>${u}</option>`).join("")}
              </select>
            </div>
          </div>
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <select class="form-select" id="case-owner" required>
                ...
                ${o.filter(u=>u.role!=="متدرب").map(u=>`<option value="${u.id}" ${a?.ownerId===u.id?"selected":""}>${u.name} (${u.role})</option>`).join("")}
              </select>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="case-status">
              ${Re.map(u=>`<option value="${u}" ${a?.status===u?"selected":""}>${u}</option>`).join("")}
            </select>
          </div>
          `:""}
  ...
  `,document.getElementById("case-type").addEventListener("change",u=>{const b=document.getElementById("criminal-stage-group");b.style.display=u.target.value==="جنائي"?"block":"none"});let p=a?.clientIds?[...a.clientIds]:a?.clientId?[a.clientId]:[],r=a?.primaryClientId||a?.clientId||"";function f(){const u=document.getElementById("client-tags-container"),b=document.getElementById("primary-client-group"),g=document.getElementById("primary-client-radios");u.innerHTML=p.map(x=>{const y=i.find(n=>n.id===x),h=r===x;return y?`<span class="client-tag ${h?"primary":""}" data-client-id="${x}">${y.name}${h?" (رئيسي)":""}<button class="client-tag-remove" data-remove-id="${x}">&times;</button></span>`:""}).join(""),u.querySelectorAll(".client-tag-remove").forEach(x=>{x.addEventListener("click",y=>{y.preventDefault();const h=x.dataset.removeId;p=p.filter(n=>n!==h),r===h&&(r=p[0]||""),f()})}),p.length>1?(b.style.display="block",g.innerHTML=p.map(x=>{const y=i.find(h=>h.id===x);return y?`<label class="primary-select-radio"><input type="radio" name="primary-client" value="${x}" ${r===x?"checked":""} />${y.name}</label>`:""}).join(""),g.querySelectorAll('input[type="radio"]').forEach(x=>{x.addEventListener("change",()=>{r=x.value,f()})})):(b.style.display="none",p.length===1&&(r=p[0]))}document.getElementById("add-client-btn").addEventListener("click",()=>{const u=document.getElementById("add-client-select"),b=u.value;b&&(p.includes(b)||(p.push(b),p.length===1&&(r=b),u.value="",f()))}),f(),document.getElementById("case-form").addEventListener("submit",u=>{u.preventDefault();const b=bt({caseNo:document.getElementById("case-no").value.trim(),year:document.getElementById("case-year").value.trim(),stageType:document.getElementById("case-stage").value,clientId:r,clientIds:[...p],primaryClientId:r,clientRole:document.getElementById("case-client-role").value,opponentName:document.getElementById("case-opponent").value.trim(),opponentRole:document.getElementById("case-opponent-role").value,court:document.getElementById("case-court").value.trim(),circuit:document.getElementById("case-circuit").value.trim(),caseType:document.getElementById("case-type").value,subject:document.getElementById("case-subject").value.trim(),firstSessionDate:document.getElementById("case-first-session").value,ownerId:document.getElementById("case-owner").value,status:s?document.getElementById("case-status")?.value||a.status:"نشطة",criminalStageType:document.getElementById("case-criminal-stage")?.value||"",notes:document.getElementById("case-notes").value.trim()}),g=[];if(b.caseNo||g.push("رقم القضية مطلوب"),b.year||g.push("السنة مطلوبة"),b.stageType||g.push("نوع المرحلة مطلوب"),p.length===0&&g.push("يجب إضافة عميل واحد على الأقل"),p.length>1&&!r&&g.push("يجب اختيار العميل الرئيسي عند وجود عدة عملاء"),b.clientRole||g.push("صفة العميل مطلوبة"),b.opponentName||g.push("اسم الخصم مطلوب"),b.opponentRole||g.push("صفة الخصم مطلوبة"),b.court||g.push("المحكمة مطلوبة"),b.circuit||g.push("الدائرة مطلوبة"),b.caseType||g.push("نوع القضية مطلوب"),b.subject||g.push("موضوع القضية مطلوب"),b.firstSessionDate||g.push("تاريخ أول جلسة مطلوب"),b.ownerId||g.push("المحامي المسؤول مطلوب"),b.caseType==="جنائي"&&!b.criminalStageType&&g.push("مرحلة القضية الجنائية مطلوبة"),s&&b.status==="مغلقة"){const x=c.query(l.ACTIONS,h=>h.caseId===t.id&&h.caseId!==""&&h.status!=="مكتمل"),y=c.query(l.DEADLINES,h=>h.caseId===t.id&&h.status==="مفتوح");x.length>0&&g.push(`لا يمكن إغلاق القضية: يوجد ${x.length} إجراء مفتوح مرتبط بها`),y.length>0&&g.push(`لا يمكن إغلاق القضية: يوجد ${y.length} موعد نهائي مفتوح`)}if(g.length>0){const x=document.getElementById("case-form-errors");x.style.display="block",x.innerHTML=g.join("<br>"),k("يرجى تصحيح الأخطاء","error");return}if(s)c.update(l.CASES,t.id,b),N(l.CASES,t.id,"update",b),k("تم تحديث القضية","success"),window.location.hash=`/cases/${t.id}`;else{const x=c.create(l.CASES,b);N(l.CASES,x.id,"create",b);const y=c.create(l.SESSIONS,{caseId:x.id,date:b.firstSessionDate,sessionType:b.caseType==="جنائي"&&b.criminalStageType==="تحقيقات نيابة"?"تحقيق":"جلسة استماع",decisionResult:"",nextSessionDate:"",notes:"جلسة أولى – تم إنشاؤها تلقائياً"});N(l.SESSIONS,y.id,"create",{auto:!0,caseId:x.id}),k("تم إنشاء القضية وجلستها الأولى بنجاح","success"),window.location.hash=`/cases/${x.id}`}})}const Dt=[{decisionType:"تأجيل لإعادة الإعلان",actionType:"إعلان/خدمة",executionProof:"تاريخ التقديم للمحضر + رقم المرجع + النتيجة",subTasks:[],requiresNextDate:!0},{decisionType:"تأجيل لتصريح",actionType:"تصريح محكمة",executionProof:"رقم التصريح + التاريخ + المرفق",subTasks:[],requiresNextDate:!0},{decisionType:"تأجيل لمذكرة ومستندات",actionType:"حزمة تحضير",executionProof:"تفاصيل التقديم",subTasks:[{title:"صياغة المذكرة",completed:!1},{title:"مراجعة المذكرة",completed:!1},{title:"تحضير المستندات",completed:!1},{title:"تصوير ونسخ",completed:!1},{title:"تقديم الحزمة",completed:!1}],requiresNextDate:!0},{decisionType:"إحالة لخبير",actionType:"متابعة خبير",executionProof:"متابعة الموعد + تقديم الملاحظات + استلام التقرير",subTasks:[{title:"متابعة موعد الخبير",completed:!1},{title:"تقديم ملاحظات",completed:!1},{title:"استلام التقرير",completed:!1}],requiresNextDate:!0},{decisionType:"شطب",actionType:"تجديد من الشطب",executionProof:"تقديم طلب التجديد",subTasks:[],requiresNextDate:!1},{decisionType:"صدور حكم",actionType:"مراجعة حكم",executionProof:"مراجعة الحكم وتحديد الإجراء التالي",subTasks:[],requiresNextDate:!1},{decisionType:"حبس احتياطي",actionType:"حضور تجديد حبس",executionProof:"حضور جلسة التجديد",subTasks:[],requiresNextDate:!1,urgent:!0},{decisionType:"طلب تحقيقات",actionType:"متابعة تحقيق",executionProof:"استلام التحقيق + الخطوة التالية",subTasks:[],requiresNextDate:!0},{decisionType:"تأجيل للمرافعة",actionType:"حزمة تحضير",executionProof:"تحضير المرافعة",subTasks:[{title:"تحضير نقاط المرافعة",completed:!1},{title:"مراجعة القضية",completed:!1}],requiresNextDate:!0},{decisionType:"تأجيل للاطلاع",actionType:"حزمة تحضير",executionProof:"الاطلاع والتحضير",subTasks:[{title:"الاطلاع على المستندات",completed:!1},{title:"تحضير الرد",completed:!1}],requiresNextDate:!0},{decisionType:"تأجيل عام",actionType:"أخرى",executionProof:"",subTasks:[],requiresNextDate:!0},{decisionType:"إحالة للمحكمة",actionType:"أخرى",executionProof:"إنشاء قضية جديدة مرتبطة",subTasks:[],requiresNextDate:!1,createsLinkedCase:!0},{decisionType:"نطق بالحكم",actionType:"مراجعة حكم",executionProof:"مراجعة الحكم وتحديد الإجراء التالي",subTasks:[],requiresNextDate:!1}];function ge(){const e=c.getAll(l.DECISION_MAP);return e.length===0?(Dt.forEach(t=>{c.create(l.DECISION_MAP,t)}),c.getAll(l.DECISION_MAP)):e}function pe(e){return ge().find(s=>s.decisionType===e)||null}function At(e,t){return c.update(l.DECISION_MAP,e,t)}function Ct(e){return c.create(l.DECISION_MAP,e)}function Bt(e){return c.softDelete(l.DECISION_MAP,e)}function Nt(e){const t=pe(e);return t?t.createsLinkedCase===!0:!1}function oe(e,t){c.getAll(e).length===0&&t.forEach((a,i)=>{c.create(e,{label:a,order:i})})}function te(){return oe(l.LOOKUP_ACTION_TYPES,Ye),c.getAll(l.LOOKUP_ACTION_TYPES).sort((e,t)=>(e.order??0)-(t.order??0)).map(e=>e.label)}function Ze(){return oe(l.LOOKUP_DECISION_TYPES,Ge),c.getAll(l.LOOKUP_DECISION_TYPES).sort((e,t)=>(e.order??0)-(t.order??0)).map(e=>e.label)}function qt(){return oe(l.LOOKUP_ACTION_TYPES,Ye),c.getAll(l.LOOKUP_ACTION_TYPES).sort((e,t)=>(e.order??0)-(t.order??0))}function Ot(){return oe(l.LOOKUP_DECISION_TYPES,Ge),c.getAll(l.LOOKUP_DECISION_TYPES).sort((e,t)=>(e.order??0)-(t.order??0))}function jt(e,t){const a=c.getAll(e).reduce((i,o)=>Math.max(i,o.order??0),0);return c.create(e,{label:t.trim(),order:a+1})}function Pt(e,t,s){return c.update(e,t,{label:s.trim()})}function Mt(e,t){const s=c.getById(e,t);if(!s)return{ok:!1};const a=ge();let i=null;return e===l.LOOKUP_ACTION_TYPES?i=a.some(o=>o.actionType===s.label):i=a.some(o=>o.decisionType===s.label),c.softDelete(e,t),i?{ok:!0,warning:`"${s.label}" محذوف من القائمة لكنه لا يزال مرتبطاً بربط قرارات. يُنصح بمراجعة صفحة ربط القرارات.`}:{ok:!0}}function Qe(e,t){if(!ne()){k("تعديل الإجراءات متاح للشركاء فقط","error");return}const s=c.getById(l.ACTIONS,e);if(!s)return;const a=c.getAll(l.CLIENTS),i=c.getAll(l.CASES),p=c.getAll(l.USERS).filter(y=>y.active&&ve.includes(y.role)),r=s.status==="مكتمل",f=te();function u(y){return y?i.filter(h=>(h.clientIds||(h.clientId?[h.clientId]:[])).includes(y)||h.primaryClientId===y||h.clientId===y):[]}const b=u(s.clientId),g=`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>`,large:!0}),document.getElementById("ea-client")?.addEventListener("change",()=>{const y=document.getElementById("ea-client").value,h=document.getElementById("ea-case");if(!h)return;const n=u(y);h.innerHTML='<option value="">بدون قضية (مستوى العميل)</option>'+n.map(v=>`<option value="${v.id}">${v.caseNo}/${v.year} – ${v.subject}</option>`).join("")}),document.getElementById("ea-save-btn").addEventListener("click",()=>{const y=document.getElementById("ea-action-type").value,h=document.getElementById("ea-title").value.trim(),n=document.getElementById("ea-priority").value,v=document.getElementById("ea-client").value,$=document.getElementById("ea-case")?.value||"",d=document.getElementById("ea-responsible").value,I=document.getElementById("ea-due-date").value,S=document.getElementById("ea-notes").value.trim(),L=r&&document.getElementById("ea-exec-date")?.value||s.executionDate,D=r&&document.getElementById("ea-exec-details")?.value?.trim()||s.executionDetails,A=document.getElementById("ea-edit-reason").value.trim(),T=[s.actionType!==y,s.responsibleUserId!==d,s.clientId!==v,s.caseId!==$,r&&(s.executionDate!==L||s.executionDetails!==D)].some(Boolean),m=[];if(v||m.push("العميل مطلوب – لا يمكن إزالة ربط الإجراء بعميل"),y||m.push("نوع الإجراء مطلوب"),d||m.push("المحامي المسؤول مطلوب"),T&&!A&&m.push("سبب التعديل مطلوب عند تغيير الحقول الحساسة"),$){const E=c.getById(l.CASES,$);E&&((E.clientIds||(E.clientId?[E.clientId]:[])).includes(v)||E.primaryClientId===v||E.clientId===v||m.push("القضية المختارة لا تنتمي للعميل المحدد"))}if(m.length>0){const E=document.getElementById("ea-errors");E.style.display="block",E.innerHTML=m.join("<br>");return}const w={actionType:y,title:h,priority:n,clientId:v,caseId:$,responsibleUserId:d,dueDate:I,notes:S,executionDate:L,executionDetails:D};St(e,s,w,A),c.update(l.ACTIONS,e,w),k("تم حفظ التعديلات بنجاح","success"),O(),typeof t=="function"&&t()})}function Ut(e,t){const s=c.getById(l.ACTIONS,e);if(!s)return;if(s.status==="مكتمل"){k("هذا الإجراء مكتمل بالفعل ولا يمكن تعديل تقدمه","warning");return}const a=`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>`});const o=document.getElementById("pu-status"),p=document.getElementById("pu-completion-fields");o?.addEventListener("change",()=>{p.style.display=o.value==="مكتمل"?"block":"none"}),document.getElementById("pu-save-btn").addEventListener("click",()=>{const r=document.getElementById("pu-status").value,f=document.getElementById("pu-notes").value.trim(),u=document.getElementById("pu-exec-date")?.value||"",b=document.getElementById("pu-exec-details")?.value?.trim()||"",g=[];if(r==="مكتمل"&&(u||g.push("تاريخ التنفيذ مطلوب لإكمال الإجراء"),b||g.push("تفاصيل التنفيذ / الإثبات مطلوبة لإكمال الإجراء")),g.length>0){const n=document.getElementById("pu-errors");n.style.display="block",n.innerHTML=g.join("<br>");return}const x={status:r,notes:f};r==="مكتمل"&&(x.executionDate=u,x.executionDetails=b),c.update(l.ACTIONS,e,x);const y=r==="مكتمل"?"complete":"status_change";N(l.ACTIONS,e,y,{oldStatus:s.status,newStatus:r,notes:f});const h=r==="مكتمل"?"تم إكمال الإجراء ✓":`تم تحديث الحالة إلى: ${r}`;k(h,"success"),O(),typeof t=="function"&&t()})}function K(e,t={}){const s=c.getById(l.CASES,t.id);if(!s){e.innerHTML=`<div class="empty-state"><h3>القضية غير موجودة</h3><button class="btn btn-primary" onclick="window.location.hash='/cases'">العودة للقضايا</button></div>`;return}const a=c.getById(l.CLIENTS,s.primaryClientId||s.clientId),p=(s.clientIds||(s.clientId?[s.clientId]:[])).map(n=>c.getById(l.CLIENTS,n)).filter(Boolean).length>1?`${a?a.name:"—"} وآخرون`:a?a.name:"—",r=c.getById(l.USERS,s.ownerId),f=c.query(l.SESSIONS,n=>n.caseId===t.id).sort((n,v)=>new Date(v.date)-new Date(n.date)),u=c.query(l.ACTIONS,n=>n.caseId===t.id),b=c.query(l.DEADLINES,n=>n.caseId===t.id),g=c.getAll(l.USERS),x=u.filter(n=>n.status!=="مكتمل");b.filter(n=>n.status==="مفتوح");const y=me[s.caseType]||"civil",h=He[s.status]||"active";j(`القضية ${s.caseNo}/${s.year}`),e.innerHTML=`
    <div class="animate-fade-in">
      <!-- Case Header -->
        ...
        <div class="case-detail-info">
          ...
          <div class="detail-grid">
            ...
            <div class="detail-item">
              ...
              <div class="detail-item-value">${r?r.name:"—"}</div>
            </div>
          </div>
        </div>
      ...
      <div class="tabs">
        ...
        <button class="tab-btn" data-tab="actions">
          <i class='bx bxs-zap'></i> الإجراءات <span class="tab-count">${u.length}</span>
        </button>
      ...
      <div class="tab-panel active" id="tab-sessions">
        ...
        <div class="timeline" id="sessions-timeline">
          ...
          ${f.map(n=>{const v=n.sessionType==="تحقيق",$=n.decisionResult?.includes("حكم"),d=n.status==="مغلق";return`
              <div class="timeline-item">
                <div class="timeline-dot ${$?"judgment":""} ${v?"investigation":""}"></div>
                <div class="timeline-content">
                  ...
                  <div class="flex gap-2 mt-2">
                    ...
                    ${d?"":`<button class="btn btn-primary btn-sm close-session-btn" data-id="${n.id}">✓ إغلاق الجلسة</button>`}
                  </div>
                </div>
              </div>
            `}).join("")}
      ...
      <div class="tab-panel" id="tab-actions">
        <div class="flex justify-between items-center mb-4">
          <h3>الإجراءات</h3>
          <div class="flex gap-2">
            ${x.length>0?`<span class="badge badge-progress">${x.length} مفتوح</span>`:""}
            <button class="btn btn-primary btn-sm" id="create-action-btn"><i class='bx bx-plus'></i> إنشاء إجراء</button>
          </div>
        </div>
        ${u.length===0?'<div class="empty-state"><p>لا توجد إجراءات بعد</p></div>':""}
        ${u.map(n=>{const v=c.getById(l.USERS,n.responsibleUserId),$=n.clientId?c.getById(l.CLIENTS,n.clientId):null,d=Fe[n.status]||"open",I=n.dueDate&&ie(n.dueDate)&&n.status!=="مكتمل",S=ne(),L=U(),D=n.status!=="مكتمل"&&(ne()||L&&n.responsibleUserId===L.id),A=wt(n.id),T=A.length===0?'<div class="text-xs text-secondary" style="padding:var(--space-2) 0">لا توجد سجلات تعديل</div>':A.map(m=>{const w=m.changes,E=new Date(m.timestamp).toLocaleString("ar-EG",{dateStyle:"short",timeStyle:"short"});if(m.action==="field_change"&&w&&w.field)return`<div class="action-history-entry">
            ...
            <div class="card mb-4 ${I?"risk-flag high":""}" style="border-right: 3px solid ${n.status==="مكتمل"?"var(--status-completed)":n.status==="معلق"?"var(--status-blocked)":"var(--status-progress)"};"
              ...
              <div class="flex justify-between items-center mb-2">
                <div class="flex items-center gap-3">
                  ...
                  <span class="badge badge-${d}">${n.status}</span>
      ...
      <div class="tab-panel" id="tab-deadlines">
        ...
        ${b.map(n=>{const v=c.getById(l.USERS,n.responsibleUserId),$=ze[n.status]||"open",d=J(n.endDate),I=n.status==="مفتوح"&&d<0,S=n.status==="مفتوح"&&d>=0&&d<=3;return`
            <div class="card mb-4" style="border-right: 3px solid ${I?"var(--risk-high)":S?"var(--risk-medium)":n.status==="مكتمل"?"var(--status-completed)":"var(--status-open)"};">
              ...
              ${n.status==="مفتوح"?`<div class="text-xs mt-2 ${I?"text-accent":""}">${I?`متأخر بـ ${Math.abs(d)} يوم`:d===0?"اليوم!":`متبقي ${d} يوم`}</div>`:""}
  ...
  `,e.querySelectorAll(".tab-btn").forEach(n=>{n.addEventListener("click",()=>{e.querySelectorAll(".tab-btn").forEach(v=>v.classList.remove("active")),e.querySelectorAll(".tab-panel").forEach(v=>v.classList.remove("active")),n.classList.add("active"),document.getElementById(`tab-${n.dataset.tab}`).classList.add("active")})}),e.querySelector("#add-session-btn")?.addEventListener("click",()=>{de(t.id,s,g,e,t)}),e.querySelector("#create-action-btn")?.addEventListener("click",()=>{const n=s.primaryClientId||s.clientId||"";_t(t.id,n,s,g,e,t)}),e.querySelectorAll(".edit-session-btn").forEach(n=>{n.addEventListener("click",()=>{const v=c.getById(l.SESSIONS,n.dataset.id);v&&de(t.id,s,g,e,t,v,!1)})}),e.querySelectorAll(".close-session-btn").forEach(n=>{n.addEventListener("click",()=>{const v=c.getById(l.SESSIONS,n.dataset.id);v&&de(t.id,s,g,e,t,v,!0)})}),e.querySelectorAll(".complete-action-btn").forEach(n=>{n.addEventListener("click",()=>{Rt(n.dataset.id,e,t)})}),e.querySelectorAll(".edit-action-btn").forEach(n=>{n.addEventListener("click",()=>{Qe(n.dataset.id,()=>K(e,t))})}),e.querySelectorAll(".action-history-toggle").forEach(n=>{n.addEventListener("click",()=>{const v=document.getElementById(n.dataset.target);if(!v)return;const $=v.style.display==="none";v.style.display=$?"block":"none",n.classList.toggle("open",$)})}),e.querySelectorAll(".subtask-check").forEach(n=>{n.addEventListener("change",()=>{const v=c.getById(l.ACTIONS,n.dataset.actionId);if(v){const $=parseInt(n.dataset.idx);v.subTasks[$].completed=n.checked,c.update(l.ACTIONS,v.id,{subTasks:v.subTasks}),N(l.ACTIONS,v.id,"update",{subTaskIndex:$,completed:n.checked})}})}),e.querySelector("#add-deadline-btn")?.addEventListener("click",()=>{Ht(t.id,g,e,t)}),e.querySelectorAll(".complete-deadline-btn").forEach(n=>{n.addEventListener("click",()=>{Ft(n.dataset.id,e,t)})})}function _t(e,t,s,a,i,o){const p=a.filter(u=>u.active&&ve.includes(u.role)),r=`
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select class="form-select" id="ca-action-type" required>
            <option value="">اختر النوع</option>
            ${te().map(u=>`<option value="${u}">${u}</option>`).join("")}
          </select>
        </div>
        <div class="form-group">
          <label class="form-label">الأولوية</label>
          <select class="form-select" id="ca-priority">
            <option value="">بدون أولوية</option>
            ${be.map(u=>`<option value="${u}">${u}</option>`).join("")}
          </select>
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select class="form-select" id="ca-responsible" required>
            ...
            ${p.map(u=>`<option value="${u.id}" ${u.id===s.ownerId?"selected":""}>${u.name} (${u.role})</option>`).join("")}
          </select>
  ...
  `,large:!0}),document.getElementById("save-create-action-btn").addEventListener("click",()=>{const u=document.getElementById("ca-action-type").value,b=document.getElementById("ca-title").value.trim(),g=document.getElementById("ca-priority").value,x=document.getElementById("ca-responsible").value,y=document.getElementById("ca-due-date").value,h=document.getElementById("ca-notes").value.trim(),n=[];if(u||n.push("نوع الإجراء مطلوب"),x||n.push("المحامي المسؤول مطلوب – يجب اختياره"),n.length>0){const d=document.getElementById("ca-errors");d.style.display="block",d.innerHTML=n.join("<br>");return}const v=ye({clientId:t,caseId:e,sessionId:"",actionType:u,title:b,priority:g,responsibleUserId:x,status:"مفتوح",dueDate:y,notes:h}),$=c.create(l.ACTIONS,v);N(l.ACTIONS,$.id,"create",{manual:!0,actionType:u,responsibleUserId:x,caseId:e}),k(`تم إنشاء الإجراء: ${u}`,"success"),O(),K(i,o)})}function de(e,t,s,a,i,o=null,p=!1){const r=!!o,u=t.caseType==="جنائي"&&t.criminalStageType==="تحقيقات نيابة"?"التحقيق":"الجلسة",b=p,g=`
    <form id="session-modal-form">
      <div class="form-row">
        <div class="form-group">
          <label class="form-label">تاريخ ${u} <span class="required">*</span></label>
          <input type="date" class="form-input" id="session-date" value="${o?.date||""}" required />
        </div>
        <div class="form-group">
          <label class="form-label">نوع ${u} <span class="required">*</span></label>
      ...
      <div class="form-group">
        ...
        <select class="form-select" id="session-decision" required>
          ...
          ${Ze().map(n=>`<option value="${n}" ${o?.decisionResult===n?"selected":""}>${n}</option>`).join("")}
        </select>
      </div>
  ...
  `,y=`
    <button class="btn btn-primary" id="save-session-btn">${b?"✓ حفظ وإغلاق الجلسة":r?"💾 حفظ":"✓ حفظ الجلسة وإنشاء الإجراء"}</button>
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
  `;M(`${b?"إغلاق":r?"تعديل":"إضافة"} ${u}`,g,{footer:y,large:!0});function h(){const n=document.getElementById("session-decision").value,v=pe(n),$=document.getElementById("session-action-preview"),d=document.getElementById("action-preview-text"),I=document.getElementById("next-date-required"),S=document.getElementById("closure-reason-group"),L=document.getElementById("next-date-group"),D=v?v.requiresNextDate:!1;v?($.style.display="block",d.textContent=`سيتم إنشاء إجراء تلقائي: ${v.actionType}`,v.subTasks?.length>0&&(d.textContent+=` (${v.subTasks.length} مهام فرعية)`)):$.style.display="none",D?(L.style.display="block",I.style.display="inline",S.style.display="none"):n?(L.style.display="block",I.style.display="none",b?S.style.display="block":S.style.display="none"):(L.style.display="block",I.style.display="inline",S.style.display="none")}document.getElementById("session-decision").addEventListener("change",h),h(),document.getElementById("save-session-btn").addEventListener("click",()=>{const n=document.getElementById("session-date").value,v=document.getElementById("session-type").value,$=document.getElementById("session-decision").value,d=document.getElementById("session-next-date").value,I=document.getElementById("session-closure-reason")?.value||"",S=document.getElementById("session-notes").value,L=[];n||L.push("تاريخ الجلسة مطلوب"),v||L.push("نوع الجلسة مطلوب"),b&&!$&&L.push("لا يمكن إغلاق الجلسة بدون تسجيل القرار/النتيجة"),!b&&!$&&L.push("نتيجة القرار مطلوبة");const D=pe($),A=D?D.requiresNextDate:!1;if(A&&!d&&L.push("تاريخ الجلسة التالية مطلوب لهذا النوع من القرار"),b&&!A&&$&&!d&&!I&&L.push("يجب اختيار سبب عدم وجود جلسة تالية لإغلاق الجلسة"),L.length>0){const B=document.getElementById("session-form-errors");B.style.display="block",B.innerHTML=L.join("<br>");return}const T=b?"مغلق":o?.status||"مفتوح",m=we({caseId:e,date:n,sessionType:v,decisionResult:$,nextSessionDate:d,status:T,closureReason:b?I:o?.closureReason||"",notes:S});let w;if(r?(c.update(l.SESSIONS,o.id,m),N(l.SESSIONS,o.id,"update",m),w={...o,...m}):(w=c.create(l.SESSIONS,m),N(l.SESSIONS,w.id,"create",m)),d&&c.query(l.SESSIONS,C=>C.caseId===e&&C.date===d&&C.id!==w.id).length===0){const C=we({caseId:e,date:d,sessionType:v,decisionResult:"",nextSessionDate:"",status:"مفتوح",closureReason:"",notes:"جلسة تالية – تم إنشاؤها تلقائياً"}),R=c.create(l.SESSIONS,C);N(l.SESSIONS,R.id,"create",{auto:!0,fromSession:w.id})}let E=!1;if(D)try{if(c.query(l.ACTIONS,C=>C.sessionId===w.id&&C.actionType===D.actionType).length===0){const C=c.getById(l.CASES,e),R=ye({caseId:e,sessionId:w.id,actionType:D.actionType,responsibleUserId:C?.ownerId||"",status:"مفتوح",subTasks:D.subTasks?D.subTasks.map(F=>({...F})):[],dueDate:d||"",notes:D.executionProof?`إثبات التنفيذ المطلوب: ${D.executionProof}`:""}),H=c.create(l.ACTIONS,R);N(l.ACTIONS,H.id,"create",{auto:!0,decision:$,sessionId:w.id}),E=!0,console.log("Action automatically created:",H)}else console.log("Action of this type already exists for this session, skipping creation.")}catch(B){console.error("Error creating action:",B),k("حدث خطأ أثناء إنشاء الإجراء التلقائي","error")}k(E?`تم ${b?"إغلاق":"حفظ"} الجلسة وإنشاء إجراء: ${D.actionType}`:b?"تم إغلاق الجلسة بنجاح":"تم حفظ الجلسة","success"),!r&&Nt($)&&c.getById(l.CASES,e)&&k("يمكنك الآن إنشاء قضية محكمة مرتبطة من صفحة القضايا","info",5e3),O(),K(a,i)})}function Rt(e,t,s){const a=c.getById(l.ACTIONS,e);if(!a)return;const i=`
  ...
  `;M("إكمال الإجراء",i,{footer:`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
  `}),document.getElementById("confirm-complete-action").addEventListener("click",()=>{const p=document.getElementById("action-exec-date").value,r=document.getElementById("action-exec-details").value.trim();if(!p||!r){const f=document.getElementById("complete-action-errors");f.style.display="block",f.innerHTML="تاريخ التنفيذ وتفاصيل التنفيذ مطلوبان",k("لا يمكن إكمال الإجراء بدون بيانات التنفيذ","error");return}c.update(l.ACTIONS,e,{status:"مكتمل",executionDate:p,executionDetails:r}),N(l.ACTIONS,e,"complete",{executionDate:p}),k("تم إكمال الإجراء بنجاح","success"),O(),K(t,s)})}function Ht(e,t,s,a){const i=`
    <form id="deadline-modal-form">
      <div class="form-group">
        ...
        <select class="form-select" id="deadline-type" required>
          <option value="">اختر النوع</option>
          ${Je.map(p=>`<option value="${p}">${p}</option>`).join("")}
        </select>
      </div>
      <div class="form-row">
        <div class="form-group">
  ...
  `;M("إضافة موعد نهائي",i,{footer:`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
  `}),document.getElementById("save-deadline-btn").addEventListener("click",()=>{const p=document.getElementById("deadline-type").value,r=document.getElementById("deadline-start").value,f=document.getElementById("deadline-end").value,u=document.getElementById("deadline-responsible").value;if(!p||!r||!f||!u){document.getElementById("deadline-form-errors").style.display="block",document.getElementById("deadline-form-errors").innerHTML="جميع الحقول مطلوبة";return}const b=vt({caseId:e,deadlineType:p,startDate:r,endDate:f,responsibleUserId:u}),g=c.create(l.DEADLINES,b);N(l.DEADLINES,g.id,"create",b),k("تم إضافة الموعد النهائي","success"),O(),K(s,a)})}function Ft(e,t,s){M("إكمال الموعد النهائي",`
    ...
    <div class="animate-fade-in">
      ...
      <div class="filter-bar">
        ...
        <select class="filter-select" id="filter-responsible">
          ...
          ${r.map(n=>`<option value="${n.id}">${n.name} (${n.role})</option>`).join("")}
        </select>`}
      </div>

      <div id="actions-container"></div>
    </div>
  `;function b(){const n=document.getElementById("action-search").value.toLowerCase(),v=document.getElementById("filter-action-type").value,$=document.getElementById("filter-action-status").value,d=document.getElementById("filter-action-scope").value,I=a?"":document.getElementById("filter-responsible")?.value||"";let S=i;v&&(S=S.filter(A=>A.actionType===v)),$&&(S=S.filter(A=>A.status===$)),d==="case"&&(S=S.filter(A=>!!A.caseId)),d==="client"&&(S=S.filter(A=>!A.caseId)),I&&(S=S.filter(A=>A.responsibleUserId===I)),n&&(S=S.filter(A=>{const T=A.caseId?c.getById(l.CASES,A.caseId):null,m=A.clientId?c.getById(l.CLIENTS,A.clientId):null;return A.actionType.toLowerCase().includes(n)||A.title&&A.title.toLowerCase().includes(n)||A.notes&&A.notes.toLowerCase().includes(n)||T&&(T.caseNo.includes(n)||T.subject.toLowerCase().includes(n))||m&&m.name.toLowerCase().includes(n)}));const L={};S.forEach(A=>{const T=A.actionType||"غير محدد";L[T]||(L[T]=[]),L[T].push(A)});const D=document.getElementById("actions-container");if(Object.keys(L).length===0){D.innerHTML='<div class="empty-state"><p>لا توجد إجراءات</p></div>';return}D.innerHTML=Object.entries(L).map(([A,T])=>`
    ...
    `).join("")}b();function g(){document.querySelectorAll(".action-edit-btn").forEach(n=>{n.addEventListener("click",()=>{Qe(n.dataset.id,()=>Q(e))})}),document.querySelectorAll(".action-progress-btn").forEach(n=>{n.addEventListener("click",()=>{Ut(n.dataset.id,()=>Q(e))})})}const x=b;function y(){x(),g()}document.getElementById("action-search").removeEventListener("input",b),document.getElementById("action-search").addEventListener("input",y),document.getElementById("filter-action-type").removeEventListener("change",b),document.getElementById("filter-action-type").addEventListener("change",y),document.getElementById("filter-action-status").removeEventListener("change",b),document.getElementById("filter-action-status").addEventListener("change",y),document.getElementById("filter-action-scope").removeEventListener("change",b),document.getElementById("filter-action-scope").addEventListener("change",y),a||document.getElementById("filter-responsible")?.addEventListener("change",y),g(),e.querySelector("#global-create-action-btn")?.addEventListener("click",()=>{zt(u,o,r,a,t,e)});function h(){Q(e)}e._refreshActionList=h}function zt(e,t,s,a,i,o){const p=s.filter(h=>h.active&&ve.includes(h.role)),f=`
  ...
  `;M("إنشاء إجراء جديد",f,{footer:`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
  `,large:!0});const b=document.getElementById("gca-client-search"),g=document.getElementById("gca-client"),x=document.getElementById("gca-case"),y=document.getElementById("gca-case-hint");Array.from(g.options),b.addEventListener("input",()=>{const h=b.value.trim().toLowerCase();g.innerHTML='<option value="">اختر العميل...</option>',e.filter(v=>v.name.toLowerCase().includes(h)).forEach(v=>{const $=document.createElement("option");$.value=v.id,$.textContent=v.name,g.appendChild($)})}),g.addEventListener("change",()=>{const h=g.value;if(x.innerHTML="",x.disabled=!0,!h){x.innerHTML='<option value="">اختر العميل أولاً...</option>',y.textContent="اختر العميل لتحميل قضاياه";return}const n=t.filter($=>($.clientIds||($.clientId?[$.clientId]:[])).includes(h)||$.primaryClientId===h||$.clientId===h),v=a&&i?n.filter($=>$.ownerId===i.id||c.getAll(l.ACTIONS).some(d=>d.caseId===$.id&&d.responsibleUserId===i.id)):n;x.innerHTML='<option value="">بدون قضية (إجراء على مستوى العميل)</option>',v.forEach($=>{const d=document.createElement("option");d.value=$.id,d.textContent=`${$.caseNo}/${$.year} – ${$.subject}`,d.dataset.ownerId=$.ownerId||"",x.appendChild(d)}),x.disabled=!1,y.textContent=v.length===0?"لا توجد قضايا متاحة لهذا العميل":`${v.length} قضية – اختياري`}),x.addEventListener("change",()=>{const n=x.options[x.selectedIndex]?.dataset?.ownerId;if(n){const v=document.getElementById("gca-responsible");v&&Array.from(v.options).find(d=>d.value===n)&&(v.value=n)}}),document.getElementById("gca-save-btn").addEventListener("click",()=>{const h=document.getElementById("gca-client").value,n=document.getElementById("gca-case").value,v=document.getElementById("gca-action-type").value,$=document.getElementById("gca-title").value.trim(),d=document.getElementById("gca-priority").value,I=document.getElementById("gca-responsible").value,S=document.getElementById("gca-due-date").value,L=document.getElementById("gca-notes").value.trim(),D=[];if(h||D.push("العميل مطلوب – لا يمكن حفظ الإجراء بدون تحديد العميل"),v||D.push("نوع الإجراء مطلوب"),$||D.push("عنوان / وصف الإجراء مطلوب"),I||D.push("المحامي المسؤول مطلوب"),n){const E=c.getById(l.CASES,n);E&&((E.clientIds||(E.clientId?[E.clientId]:[])).includes(h)||E.primaryClientId===h||E.clientId===h||D.push("القضية المختارة لا تنتمي للعميل المحدد"))}if(D.length>0){const E=document.getElementById("gca-errors");E.style.display="block",E.innerHTML=D.join("<br>");return}const A=ye({clientId:h,caseId:n||"",sessionId:"",actionType:v,title:$,priority:d,responsibleUserId:I,status:"مفتوح",dueDate:S,notes:L}),T=c.create(l.ACTIONS,A);N(l.ACTIONS,T.id,"create",{source:"global",clientId:h,caseId:n||null,actionType:v,responsibleUserId:I});const m=c.getById(l.CLIENTS,h)?.name||"";k(`تم إنشاء الإجراء: ${v} – ${m} (${n?"ضمن القضية":"على مستوى العميل"})`,"success"),O(),Q(o)})}function Yt(e){j("المواعيد النهائية");const t=c.getAll(l.DEADLINES);e.innerHTML=`
    <div class="animate-fade-in">
      <div class="page-header">
        <div>
      ...
      <div class="filter-bar">
        <select class="filter-select" id="filter-dl-type">
          <option value="">كل الأنواع</option>
          ${Je.map(a=>`<option value="${a}">${a}</option>`).join("")}
        </select>
  ...
  `;function s(){const a=document.getElementById("filter-dl-type").value,i=document.getElementById("filter-dl-status").value;let o=t;a&&(o=o.filter(r=>r.deadlineType===a)),i&&(o=o.filter(r=>r.status===i)),o.sort((r,f)=>new Date(r.endDate)-new Date(f.endDate));const p=document.getElementById("dl-table-body");if(o.length===0){p.innerHTML='<tr><td colspan="8"><div class="empty-state"><p>لا توجد مواعيد نهائية</p></div></td></tr>';return}p.innerHTML=o.map(r=>{const f=c.getById(l.CASES,r.caseId),u=c.getById(l.USERS,r.responsibleUserId),b=ze[r.status]||"open",g=J(r.endDate),x=r.status==="مفتوح"&&g<0,y=r.status==="مفتوح"&&g>=0&&g<=3;return`
        <tr class="${x?"risk-flag high":y?"risk-flag medium":""}">
          <td><strong>${r.deadlineType}</strong></td>
          <td>
            <a href="#/cases/${r.caseId}" style="color: var(--text-link);">
              ${f?f.caseNo+"/"+f.year:"—"}
            </a>
          </td>
          <td>${P(r.startDate)}</td>
          <td>${P(r.endDate)}</td>
          <td>
            ${r.status==="مفتوح"?x?`<span class="badge badge-blocked">متأخر ${Math.abs(g)} يوم</span>`:g===0?'<span class="badge badge-progress">اليوم!</span>':`<span class="badge ${y?"badge-progress":"badge-open"}">${g} يوم</span>`:"—"}
          </td>
          <td>${u?u.name:"—"}</td>
          <td><span class="badge badge-${b}">${r.status}</span></td>
          <td>
            <button class="btn btn-ghost btn-sm" onclick="window.location.hash='/cases/${r.caseId}'">عرض ←</button>
          </td>
        </tr>
  ...
  `,e.querySelector("#add-mapping-btn").addEventListener("click",()=>{Ce(null,e)}),e.querySelectorAll(".edit-mapping").forEach(s=>{s.addEventListener("click",()=>{const a=t.find(i=>i.id===s.dataset.id);a&&Ce(a,e)})}),e.querySelectorAll(".delete-mapping").forEach(s=>{s.addEventListener("click",()=>{Bt(s.dataset.id),k("تم حذف الربط","success"),fe(e)})})}function Ce(e,t){const s=!!e,a=Ze(),i=te(),o=`
    <form id="mapping-form">
      <div class="form-group">
        ...
        <select class="form-select" id="map-decision" required>
          ...
          ${a.map(r=>`<option value="${r}" ${e?.decisionType===r?"selected":""}>${r}</option>`).join("")}
        </select>
      </div>
      <div class="form-group">
        ...
        <select class="form-select" id="map-action" required>
          <option value="">اختر</option>
          ${i.map(r=>`<option value="${r}" ${e?.actionType===r?"selected":""}>${r}</option>`).join("")}
        </select>
      </div>
      <div class="form-group">
  ...
  `;M(s?"تعديل الربط":"إضافة ربط جديد",o,{footer:`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
  `}),document.getElementById("save-mapping").addEventListener("click",()=>{const r={decisionType:document.getElementById("map-decision").value,actionType:document.getElementById("map-action").value,executionProof:document.getElementById("map-proof").value,requiresNextDate:document.getElementById("map-requires-date").checked,urgent:document.getElementById("map-urgent").checked};if(!r.decisionType||!r.actionType){k("نوع القرار والإجراء مطلوبان","error");return}s?At(e.id,r):Ct(r),k(s?"تم تحديث الربط":"تم إضافة الربط","success"),O(),fe(t)})}const he="/api";function Ie(e){if(j("إدارة المستخدمين"),!W()){e.innerHTML='<div class="empty-state"><h3>غير مصرح</h3><p>هذه الصفحة متاحة للشركاء فقط</p></div>';return}const t=c.getAll(l.USERS);e.innerHTML=`
    <div class="animate-fade-in">
      <div class="page-header">
        <div>
              ...
              <tr>
                ...
                <td>
                  <div class="table-actions">
                    ...
                    ${s.id===U()?.id?"":`<button class="btn btn-ghost btn-sm delete-user" data-id="${s.id}" data-name="${s.name}" title="حذف"><i class='bx bx-trash text-red-500'></i></button>`}
                  </div>
                </td>
              </tr>
            `).join("")}
  ...
  `,e.querySelector("#add-user-btn").addEventListener("click",()=>Be(null,e)),e.querySelectorAll(".edit-user").forEach(s=>{s.addEventListener("click",()=>{const a=c.getById(l.USERS,s.dataset.id);a&&Be(a,e)})}),e.querySelectorAll(".delete-user").forEach(s=>{s.addEventListener("click",()=>{const a=s.dataset.id,i=s.dataset.name;We("حذف المستخدم",`هل أنت متأكد من حذف المستخدم <strong>${i}</strong>؟<br><br><span style="color:var(--text-secondary);font-size:13px;">لن يتمكن هذا المستخدم من الدخول للنظام مرة أخرى.</span>`,async()=>{try{const o=G(),p=await fetch(`${he}/users/${a}`,{method:"DELETE",headers:{Authorization:`Bearer ${o}`}});if(!p.ok){const r=await p.json().catch(()=>({}));throw new Error(r.error||"فشل الحذف")}c.softDelete(l.USERS,a),N(l.USERS,a,"delete",{name:i}),k("تم حذف المستخدم بنجاح","success"),Ie(e)}catch(o){k(o.message||"حدث خطأ أثناء الاتصال بالخادم","error")}})})}),e.querySelectorAll(".set-password-btn").forEach(s=>{s.addEventListener("click",()=>{Jt(s.dataset.id,s.dataset.name)})})}async function Jt(e,t){const s=`
  ...
  `;M("تعيين كلمة مرور",s,{footer:`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
  `}),document.getElementById("adm-pw-save").addEventListener("click",async()=>{const i=document.getElementById("adm-pw").value,o=document.getElementById("adm-pw-confirm").value,p=document.getElementById("adm-pw-error");if(p.style.display="none",!i||i.length<8){p.textContent="كلمة المرور يجب أن تكون 8 أحرف على الأقل",p.style.display="block";return}if(i!==o){p.textContent="كلمتا المرور غير متطابقتين",p.style.display="block";return}const r=document.getElementById("adm-pw-save");r.disabled=!0,r.textContent="جارٍ الحفظ...";try{const f=await fetch(`${he}/auth/admin-set-password`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${G()}`},body:JSON.stringify({userId:e,password:i})}),u=await f.json();if(!f.ok){p.textContent=u.error||"حدث خطأ",p.style.display="block",r.disabled=!1,r.innerHTML="<i class='bx bx-check'></i> تعيين كلمة المرور";return}k(`✅ تم تعيين كلمة مرور ${t} بنجاح`,"success"),O()}catch{p.textContent="تعذر الاتصال بالخادم",p.style.display="block",r.disabled=!1,r.innerHTML="<i class='bx bx-check'></i> تعيين كلمة المرور"}})}function Be(e,t){const s=!!e,a=`
    <form>
      <div class="form-group">
  ...
  `;M(s?"تعديل المستخدم":"إضافة مستخدم جديد",a,{footer:`
    ...
    <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
  `}),document.getElementById("save-user").addEventListener("click",async()=>{const o=document.getElementById("user-form-error");o.style.display="none";const p=document.getElementById("user-name").value.trim(),r=document.getElementById("user-role").value,f=document.getElementById("user-email").value.trim(),u=document.getElementById("user-phone").value.trim(),b=s?document.getElementById("user-active")?.checked:!0;if(!p){o.textContent="اسم المستخدم مطلوب",o.style.display="block";return}if(s){const g=yt({name:p,role:r,email:f,phone:u,active:b});c.update(l.USERS,e.id,g),N(l.USERS,e.id,"update",g),k("تم تحديث المستخدم","success")}else{const g=document.getElementById("user-password").value,x=document.getElementById("user-password-confirm").value;if(!g||g.length<8){o.textContent="كلمة المرور يجب أن تكون 8 أحرف على الأقل",o.style.display="block";return}if(g!==x){o.textContent="كلمتا المرور غير متطابقتين",o.style.display="block";return}const y={name:p,role:r,email:f,phone:u,active:!0,password:g},h=document.getElementById("save-user");h.disabled=!0;try{const n=G(),v=await fetch(`${he}/users`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${n}`},body:JSON.stringify(y)}),$=await v.json();if(!v.ok){k(`خطأ: ${$.error||"تعذر إضافة المستخدم"}`,"error"),h.disabled=!1;return}const d=c.getAll(l.USERS);d.push($),localStorage.setItem("slf_users",JSON.stringify(d)),c.emit("change",{entityType:l.USERS,action:"create",data:$}),N(l.USERS,$.id,"create",y),k(`✅ تم إضافة ${p} وتعيين كلمة مروره بنجاح`,"success")}catch{k("تعذر الاتصال بالخادم","error"),h.disabled=!1;return}}O(),Ie(t)})}function Gt(e){if(j("سجل المراجعة"),!W()){e.innerHTML='<div class="empty-state"><h3>غير مصرح</h3><p>هذه الصفحة متاحة للشركاء فقط</p></div>';return}const t=Tt(100);e.innerHTML=`
    <div class="animate-fade-in">
      <div class="page-header">
        <div>
  ...
  `}function Kt(e){if(j("إعدادات النظام"),!W()){e.innerHTML='<div class="empty-state"><h3>غير مصرح</h3><p>هذه الصفحة متاحة للشركاء فقط</p></div>';return}ae(e)}function ae(e,t="notifications"){const s=c.getSetting("workdayEndTime")||"17:00",a=qt(),i=Ot();e.innerHTML=`
    <div class="animate-fade-in">
      ...
      <div id="tab-actions" style="display:${t==="actions"?"block":"none"}">
        ${Ne("action",a,"أنواع الإجراءات","⚡")}
      </div>

      <!-- Tab: Decision Types -->
      <div id="tab-decisions" style="display:${t==="decisions"?"block":"none"}">
        ${Ne("decision",i,"أنواع القرارات","⚖️")}
      </div>
    </div>`,e.querySelectorAll(".settings-tab-btn").forEach(o=>{o.addEventListener("click",()=>ae(e,o.dataset.tab))}),e.querySelector("#save-settings-btn")?.addEventListener("click",()=>{const o=document.getElementById("workday-end-time").value;if(!o){k("وقت نهاية يوم العمل مطلوب","error");return}c.setSetting("workdayEndTime",o),k("تم حفظ الإعدادات بنجاح","success")}),qe(e,"action",l.LOOKUP_ACTION_TYPES,()=>ae(e,"actions")),qe(e,"decision",l.LOOKUP_DECISION_TYPES,()=>ae(e,"decisions"))}function Ne(e,t,s,a){return`
    <div class="card">
      ...
      <td>
        <div class="table-actions">
          ...
          <button class="btn btn-ghost btn-sm ${e}-delete-btn" data-id="${t.id}"
    ...
    </tr>`}function qe(e,t,s,a){e.querySelector(`#${t}-search`)?.addEventListener("input",i=>{const o=i.target.value.toLowerCase();e.querySelectorAll(`.${t}-row`).forEach(p=>{const r=p.querySelector(`.${t}-label`)?.textContent?.toLowerCase()||"";p.style.display=r.includes(o)?"":"none"})}),e.querySelector(`#${t}-add-btn`)?.addEventListener("click",()=>{const i=e.querySelector(`#${t}-add-row`);i.style.display="block",e.querySelector(`#${t}-add-input`)?.focus()}),e.querySelector(`#${t}-add-cancel`)?.addEventListener("click",()=>{e.querySelector(`#${t}-add-row`).style.display="none",e.querySelector(`#${t}-add-input`).value=""}),e.querySelector(`#${t}-add-confirm`)?.addEventListener("click",()=>{const o=e.querySelector(`#${t}-add-input`)?.value?.trim();if(!o){k("الاسم مطلوب","error");return}jt(s,o),k(`تمت إضافة "${o}"`,"success"),a()}),e.querySelectorAll(`.${t}-edit-btn`).forEach(i=>{i.addEventListener("click",()=>{const o=e.querySelector(`[data-id="${i.dataset.id}"].${t}-row`);o.querySelector(`.${t}-label`).style.display="none",o.querySelector(`.${t}-edit-input`).style.display="block",o.querySelector(`.${t}-edit-btn`).style.display="none",o.querySelector(`.${t}-delete-btn`).style.display="none",o.querySelector(`.${t}-save-btn`).style.display="inline-flex",o.querySelector(`.${t}-cancel-btn`).style.display="inline-flex",o.querySelector(`.${t}-edit-input`)?.focus()})}),e.querySelectorAll(`.${t}-cancel-btn`).forEach(i=>{i.addEventListener("click",()=>{const o=e.querySelector(`[data-id="${i.dataset.id}"].${t}-row`);o.querySelector(`.${t}-label`).style.display="",o.querySelector(`.${t}-edit-input`).style.display="none",o.querySelector(`.${t}-edit-btn`).style.display="inline-flex",o.querySelector(`.${t}-delete-btn`).style.display="inline-flex",o.querySelector(`.${t}-save-btn`).style.display="none",o.querySelector(`.${t}-cancel-btn`).style.display="none"})}),e.querySelectorAll(`.${t}-save-btn`).forEach(i=>{i.addEventListener("click",()=>{const p=e.querySelector(`[data-id="${i.dataset.id}"].${t}-row`).querySelector(`.${t}-edit-input`)?.value?.trim();if(!p){k("الاسم لا يمكن أن يكون فارغاً","error");return}Pt(s,i.dataset.id,p),k("تم التحديث","success"),a()})}),e.querySelectorAll(`.${t}-delete-btn`).forEach(i=>{i.addEventListener("click",()=>{if(!confirm("هل تريد حذف هذا العنصر؟"))return;const o=Mt(s,i.dataset.id);o.ok&&(o.warning?k(o.warning,"warning",6e3):k("تم الحذف","success"),a())})})}function Wt(e){j("التقويم");let t=new Date;t.setDate(1);function s(){const i=t.getFullYear(),o=t.getMonth(),p=c.getAll(l.SESSIONS),r=c.getAll(l.DEADLINES).filter(m=>m.status==="مفتوح"),f=c.getAll(l.CASES),u=c.getAll(l.CLIENTS),b={};function g(m){if(!m)return"—";const w=m.clientIds||(m.clientId?[m.clientId]:[]),E=m.primaryClientId||m.clientId,B=u.find(C=>C.id===E);return w.length>1&&B?B.name+" وآخرون":B?B.name:"—"}function x(m){return m?m.split("T")[0]:null}function y(m,w,E,B){const C=x(m);if(!C)return;const[R,H]=C.split("-").map(Number);if(R!==i||H-1!==o)return;b[C]||(b[C]=[]);const F=f.find(_=>_.id===E),z=g(F);b[C].push({type:w,title:z,caseId:E,label:B})}p.forEach(m=>{m.date&&y(m.date,"session",m.caseId,"جلسة")}),r.forEach(m=>{m.endDate&&y(m.endDate,"deadline",m.caseId,"موعد نهائي")});const h=new Date(i,o,1),n=new Date(i,o+1,0),v=h.getDay(),$=n.getDate(),d=["يناير","فبراير","مارس","أبريل","مايو","يونيو","يوليو","أغسطس","سبتمبر","أكتوبر","نوفمبر","ديسمبر"],I=["أحد","إثنين","ثلاثاء","أربعاء","خميس","جمعة","سبت"],S=new Date().toISOString().split("T")[0],L=3;let D="";for(let m=0;m<v;m++)D+='<div class="cal-day cal-day-empty"></div>';for(let m=1;m<=$;m++){const w=String(o+1).padStart(2,"0"),E=String(m).padStart(2,"0"),B=`${i}-${w}-${E}`,C=b[B]||[],R=B===S,H=C.slice(0,L),F=C.length-L;let z=H.map(_=>`
                <div class="cal-event cal-event-${_.type}"
                     data-caseid="${_.caseId}"
                     title="${_.title} – ${_.label}"
                     style="cursor:pointer; user-select:none;">
                    <span class="cal-dot cal-dot-${_.type}"></span>
                    <span class="cal-event-title">${_.title}</span>
                    <span class="cal-event-label">${_.label}</span>
                </div>
            ...
            `}const A=[];for(let m=1;m<=$;m++){const w=String(o+1).padStart(2,"0"),E=String(m).padStart(2,"0"),B=`${i}-${w}-${E}`,C=b[B];C&&C.length>0&&A.push({dateStr:B,day:m,events:C})}const T=A.length>0?A.map(({dateStr:m,day:w,events:E})=>`<div class="cal-agenda-day">
        ...
        <div class="animate-fade-in">
            <div class="cal-header">
                ...
                <h2 class="cal-month-title">${d[o]} ${i}</h2>
        ...
        `,i.parentElement.appendChild(u),u.querySelector(".cal-popover-close").addEventListener("click",()=>u.remove()),u.querySelectorAll(".cal-popover-item").forEach(g=>{g.addEventListener("click",()=>{const x=g.dataset.caseid;x&&(window.location.hash=`/cases/${x}`),u.remove()})});const b=g=>{u.contains(g.target)||(u.remove(),document.removeEventListener("click",b))};setTimeout(()=>document.addEventListener("click",b),10)}s()}let $e=!1;function Xt(){setInterval(Oe,60*1e3),Oe()}function Oe(){const e=c.getSetting("workdayEndTime");if(!e)return;const t=new Date,[s,a]=e.split(":").map(Number),i=t.getHours()*60+t.getMinutes(),o=s*60+a;if(i<o){$e&&je();return}const p=t.toISOString().split("T")[0],f=c.getAll(l.SESSIONS).filter(u=>u.date===p&&(!u.decisionResult||u.status!=="مغلق"));f.length>0?Zt(f):je()}function Zt(e){$e=!0;const t=document.getElementById("notification-bar");if(!t)return;const s=c.getAll(l.CASES),a=c.getAll(l.CLIENTS);t.innerHTML=`
        <div class="notif-bar">
            ...
            <div class="notif-bar-list" id="notif-list" style="display: none;">
                ${e.map(i=>{const o=s.find(f=>f.id===i.caseId),p=o?o.primaryClientId||o.clientId:"",r=a.find(f=>f.id===p);return`
                        <div class="notif-item">
                            <div class="notif-item-info">
                                <div class="notif-item-client">${r?r.name:"—"}</div>
    ...
    `;const a=e.querySelector("#btn-scan"),i=e.querySelector("#drive-folder-id"),o=e.querySelector("#loading-overlay"),p=e.querySelector("#scan-results-card"),r=e.querySelector("#import-table-body"),f=e.querySelector("#btn-import-selected"),u=e.querySelector("#selectAllDetected");let b=[];a.addEventListener("click",async()=>{const y=i.value.trim();if(!y){k("الرجاء إدخال رقم تعريف المجلد (Folder ID)","error");return}c.setSetting("drive_pdf_folder_id",y),o.style.display="flex";try{const n=await fetch(`/api/scan-drive-pdfs?folderId=${encodeURIComponent(y)}`),v=await n.json();if(!n.ok)throw new Error(v.error||"حدث خطأ أثناء المسح");b=v.clients||[],b.length===0?(k("لم يتم العثور على أي بيانات عملاء في المجلد.","warning"),p.style.display="none"):(k(`تم اكتشاف ${b.length} عميل`,"success"),g(b),p.style.display="block")}catch(h){console.error(h),k(h.message,"error")}finally{o.style.display="none"}});function g(y){r.innerHTML="",y.forEach((n,v)=>{const $=document.createElement("tr");$.innerHTML=`
            ...
            `,r.appendChild($)}),x(),e.querySelectorAll(".client-checkbox").forEach(n=>{n.addEventListener("change",x)})}u.addEventListener("change",y=>{e.querySelectorAll(".client-checkbox").forEach(n=>{n.checked=y.target.checked}),x()});function x(){const y=Array.from(e.querySelectorAll(".client-checkbox")).some(h=>h.checked);f.disabled=!y}f.addEventListener("click",()=>{const y=e.querySelectorAll(".client-checkbox");let h=0;y.forEach(n=>{if(n.checked){const v=n.getAttribute("data-index"),$=e.querySelector(`#name-${v}`).value.trim(),d=e.querySelector(`#nid-${v}`).value.trim(),I=b[v],S=Ke({name:$||I.name,nationalId:d||I.nationalId,notes:`تم إضافته من المسح الآلي للملف: ${I.sourceFile}`,driveFolderUrl:I.driveFolderUrl||"",driveFolderId:I.sourceFileId||""}),L=c.create(l.CLIENTS,S);N(l.CLIENTS,L.id,"create",S),h++}}),k(`تم استيراد ${h} عميل بنجاح`,"success"),setTimeout(()=>{window.location.hash="/clients"},1500)})}const Pe="/api";function es(e){j("استيراد عملاء من Google Sheets"),e.innerHTML=`
    <div class="animate-fade-in">
      <div class="page-header">
        <div>
    ...
    `;let t={validRows:[],invalidRows:[],conflicts:[]};const s=e.querySelector("#sheet-url"),a=e.querySelector("#tab-name"),i=e.querySelector("#btn-preview"),o=e.querySelector("#loading-bar"),p=e.querySelector("#preview-section"),r=e.querySelector("#stat-valid"),f=e.querySelector("#stat-invalid"),u=e.querySelector("#stat-conflict"),b=e.querySelector("#valid-tbody"),g=e.querySelector("#invalid-tbody"),x=e.querySelector("#conflicts-tbody"),y=e.querySelector("#select-all-valid"),h=e.querySelector("#commit-card"),n=e.querySelector("#btn-commit"),v=e.querySelector("#import-mode"),$=e.querySelector("#commit-results"),d=e.querySelector("#commit-summary");e.querySelectorAll(".tab-btn").forEach(T=>{T.addEventListener("click",()=>{e.querySelectorAll(".tab-btn").forEach(m=>{m.classList.remove("btn-primary","active"),m.classList.add("btn-ghost")}),T.classList.add("btn-primary","active"),T.classList.remove("btn-ghost"),["valid","invalid","conflicts"].forEach(m=>{e.querySelector(`#tab-${m}`).style.display=T.dataset.tab===m?"":"none"})})}),i.addEventListener("click",async()=>{const T=s.value.trim();if(!T){k("أدخل رابط الجدول","error");return}o.style.display="block",p.style.display="none",$.style.display="none";try{const m=G(),w=await fetch(`${Pe}/clients/import/google-sheet/preview`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${m}`},body:JSON.stringify({sheetUrl:T,tabName:a.value.trim()||void 0})}),E=await w.json();if(!w.ok)throw new Error(E.error||"حدث خطأ أثناء المعاينة");t=E,I(E)}catch(m){console.error(m),k(m.message,"error")}finally{o.style.display="none"}});function I({validRows:T,invalidRows:m,conflicts:w}){r.textContent=`${T.length} صف صحيح`,f.textContent=`${m.length} صف خاطئ`,u.textContent=`${w.length} تعارض`,b.innerHTML=T.length===0?'<tr><td colspan="8" class="text-center" style="color:var(--text-secondary); padding:2rem;">لا توجد صفوف صحيحة</td></tr>':T.map(E=>`
                <tr>
                ...
                </tr>`).join(""),x.innerHTML=w.length===0?'<tr><td colspan="3" class="text-center" style="color:var(--text-secondary); padding:2rem;">لا توجد تعارضات ✅</td></tr>':w.map(E=>`
                <tr>
                  <td>${E.rowNumber}</td>
    ...
    `,Me("sp-password","sp-pw-toggle","sp-pw-icon"),Me("sp-confirm","sp-confirm-toggle","sp-confirm-icon"),document.getElementById("sp-password").addEventListener("input",a=>{os(a.target.value)}),document.getElementById("set-pw-form").addEventListener("submit",async a=>{a.preventDefault(),await ls(s)}),setTimeout(()=>document.getElementById("sp-password")?.focus(),100)}function Me(e,t,s){const a=document.getElementById(e),i=document.getElementById(t),o=document.getElementById(s);i.addEventListener("click",()=>{const p=a.type==="password";a.type=p?"text":"password",o.className=p?"bx bx-show":"bx bx-hide"})}function os(e){const t=document.getElementById("strength-fill"),s=document.getElementById("strength-label");let a=0;e.length>=8&&a++,/[A-Z]/.test(e)&&a++,/[0-9]/.test(e)&&a++,/[^A-Za-z0-9]/.test(e)&&a++;const i=[{pct:"0%",color:"transparent",text:""},{pct:"25%",color:"#ef4444",text:"ضعيفة جداً"},{pct:"50%",color:"#f59e0b",text:"ضعيفة"},{pct:"75%",color:"#60a5fa",text:"جيدة"},{pct:"100%",color:"#10b981",text:"قوية ✓"}],o=e.length===0?i[0]:i[Math.max(1,a)];t.style.width=o.pct,t.style.backgroundColor=o.color,s.textContent=o.text,s.style.color=o.color}async function ls(e){const t=document.getElementById("sp-password").value,s=document.getElementById("sp-confirm").value,a=document.getElementById("sp-btn");if(V("",""),t.length<8){V("error","كلمة المرور يجب أن تكون 8 أحرف على الأقل");return}if(t!==s){V("error","كلمتا المرور غير متطابقتين");return}a.classList.add("loading"),a.disabled=!0;try{const i=await fetch(`${ns}/auth/set-password`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:e,password:t})}),o=await i.json();if(!i.ok){V("error",o.error||"خطأ في تعيين كلمة المرور");return}V("success","تم تعيين كلمة المرور بنجاح! جارٍ تحويلك لتسجيل الدخول..."),setTimeout(()=>{const p=new URL(window.location.href);p.searchParams.delete("token"),window.history.replaceState({},"",p.pathname),window.location.hash="/login",window.location.reload()},1800)}catch{V("error","تعذر الاتصال بالخادم، يرجى المحاولة مجدداً")}finally{a.classList.remove("loading"),a.disabled=!1}}function V(e,t){const s=document.getElementById("sp-message");if(s){if(!t){s.style.display="none";return}s.className=e==="error"?"auth-error":"auth-success",s.innerHTML=`<i class='bx ${e==="error"?"bx-error-circle":"bx-check-circle"}'></i><span>${t}</span>`,s.style.display="flex"}}async function cs(){const t=new URLSearchParams(window.location.search).get("token"),s=window.location.hash;if(t||s==="#/set-password"){is();return}if(!ft()){ss(),window.addEventListener("auth:login",()=>{document.getElementById("auth-root").classList.add("hidden"),Ue()},{once:!0});return}await Ue()}async function Ue(){const e=[l.CLIENTS,l.CASES,l.SESSIONS,l.ACTIONS,l.DEADLINES,l.USERS];await c.syncFromServer(e),xt();let t=U();if(!t){const s=c.getAll(l.USERS);s.length>0&&(Ve(s[0]),t=s[0])}Et(),ms(),bs(),q("/",Te),q("/dashboard",Te),q("/clients",Xe),q("/clients/new",De),q("/clients/:id/edit",De),q("/clients/import",Qt),q("/clients/import-sheet",es),q("/cases",Lt),q("/cases/new",Ae),q("/cases/:id",K),q("/cases/:id/edit",Ae),q("/actions",Q),q("/deadlines",Yt),q("/calendar",Wt),q("/admin/mapping",fe),q("/admin/users",Ie),q("/admin/audit",Gt),q("/admin/settings",Kt),ds(),ot(),window.addEventListener("hashchange",()=>{et(window.location.hash.replace("#","")||"/dashboard")}),Xt(),(!window.location.hash||window.location.hash==="#")&&nt("/dashboard")}function ds(){const e=document.getElementById("topbar"),t=U();e.innerHTML=`
  ...
  `,e.querySelector("#logout-btn").addEventListener("click",()=>{confirm("هل تريد تسجيل الخروج؟")&&ht()}),e.querySelector("#change-pw-btn").addEventListener("click",us),document.getElementById("mobile-menu-btn").addEventListener("click",ps)}const rs="/api";function us(){M("تغيير كلمة المرور",`
    ...
    `,{footer:`
      ...
      <button class="btn btn-secondary" onclick="document.getElementById('active-modal')?.remove()">إلغاء</button>
    `}),document.getElementById("cp-save-btn").addEventListener("click",async()=>{const s=document.getElementById("cp-error"),a=document.getElementById("cp-success");s.style.display="none",a.style.display="none";const i=document.getElementById("cp-current").value,o=document.getElementById("cp-new").value,p=document.getElementById("cp-confirm").value;if(!i){s.textContent="أدخل كلمة المرور الحالية",s.style.display="block";return}if(!o||o.length<8){s.textContent="كلمة المرور الجديدة يجب أن تكون 8 أحرف على الأقل",s.style.display="block";return}if(o!==p){s.textContent="كلمتا المرور غير متطابقتين",s.style.display="block";return}const r=document.getElementById("cp-save-btn");r.disabled=!0,r.textContent="جارٍ الحفظ...";try{const f=await fetch(`${rs}/auth/change-password`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${G()}`},body:JSON.stringify({currentPassword:i,newPassword:o})}),u=await f.json();if(!f.ok){s.textContent=u.error||"حدث خطأ",s.style.display="block",r.disabled=!1,r.innerHTML="<i class='bx bx-check'></i> تغيير كلمة المرور";return}a.innerHTML="<i class='bx bx-check-circle'></i> ✅ تم تغيير كلمة المرور بنجاح!",a.style.display="block",r.style.display="none",setTimeout(()=>O(),2e3)}catch{s.textContent="تعذر الاتصال بالخادم",s.style.display="block",r.disabled=!1,r.innerHTML="<i class='bx bx-check'></i> تغيير كلمة المرور"}})}function ps(){const e=document.getElementById("sidebar"),t=document.getElementById("sidebar-overlay");e.classList.contains("open")?xe():(e.classList.add("open"),t&&t.classList.add("active"),document.body.style.overflow="hidden")}function xe(){const e=document.getElementById("sidebar"),t=document.getElementById("sidebar-overlay");e.classList.remove("open"),t&&t.classList.remove("active"),document.body.style.overflow=""}function ms(){if(document.getElementById("sidebar-overlay"))return;const e=document.createElement("div");e.id="sidebar-overlay",e.className="sidebar-overlay",e.addEventListener("click",xe),document.getElementById("app").appendChild(e)}function bs(){if(document.getElementById("bottom-nav"))return;const e=c.count(l.ACTIONS,s=>s.status!=="مكتمل"),t=document.createElement("nav");t.id="bottom-nav",t.className="bottom-nav",t.innerHTML=`
